...
FEATURES: tuple[BrainReadinessFeature, ...] = (
    ...
    BrainReadinessFeature(
        ...
        label="Telegram",
        express_posture="guided",
        required_user_action="Open Settings > Channels, choose Telegram, and paste a BotFather token once.",
        machine_prerequisite="A BotFather-created bot token and no competing poller for that bot.",
        config_paths=("integrations.telegram.*",),
        generated_env_keys=("START_TELEGRAM", "BOT_TOKEN"),
        health_probe="encrypted browser connection, live Telegram identity, in-process worker, real delivery, and separate operator-owned bridge status.",
        self_heal_action="Open Settings > Channels to test or reconnect; stop any competing poller before switching between browser and operator ownership.",
    ...
    BrainReadinessFeature(
        ...
        label="Slack",
        express_posture="guided",
        required_user_action="Open Settings > Channels, download the generated Slack manifest, create the app, then paste its app and bot tokens.",
        machine_prerequisite="A Slack workspace that permits app installation plus app-level and bot tokens.",
        config_paths=(),
        generated_env_keys=(),
        health_probe="encrypted browser connection, live Slack auth, Socket Mode worker, and real delivery.",
        self_heal_action="Open Settings > Channels to test, repair permissions, or reconnect the in-process worker.",
        qa_owner="qa/channel-connections/cases.md",
        public_safety_rule="Never publish Slack tokens, workspace IDs, channel IDs, or message text.",
    ),
    BrainReadinessFeature(
        key="telegram_codex",
    ...
    BrainReadinessFeature(
        ...
        label="WhatsApp",
        express_posture="guided",
        required_user_action="Open Settings > Channels and follow the Meta Cloud API setup for a business account, phone number, credentials, and webhook.",
        machine_prerequisite="Meta business/Cloud API access and a stable public HTTPS webhook edge; localhost alone cannot receive WhatsApp callbacks.",
        config_paths=("runtime.network.public_api_origin",),
        generated_env_keys=("VIVENTIUM_PUBLIC_SERVER_URL",),
        health_probe="encrypted browser connection, Meta identity, public HTTPS callback verification, and real webhook delivery.",
        self_heal_action="Open Settings > Channels to finish Meta webhook verification, repair the public edge, or reconnect.",
        qa_owner="qa/channel-connections/cases.md",
