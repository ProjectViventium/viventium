...
def test_glasshive_http_error_preserves_structured_blocker_payload() -> None:
    ...
    url = "http://127.0.0.1:8766/v1/projects/prj_1/workers/find-or-resume"
    error = urllib.error.HTTPError(
        ...
        io.BytesIO(
            json.dumps(
                {
                    "status": "blocked",
                    "detail": "GlassHive could not start the selected worker.",
    ...
    parsed = dispatch._format_http_error("POST", url, error)

    assert parsed.status == 409
    assert parsed.path == "/v1/projects/prj_1/workers/find-or-resume"
    assert parsed.failure_class == "runtime_dependency_missing"
    assert parsed.failure_retryable is False
    assert "runtime_dependency_missing" in str(parsed)
    assert "GlassHive could not start" in str(parsed)
...
def test_glasshive_find_or_resume_409_preserves_runtime_dependency_failure(
    ...
    storage.create_task(
        {
            ...
            "updated_source": "user",
            "last_run_at": None,
            "next_run_at": "2026-05-30T10:00:00Z",
            "last_status": None,
            "last_error": None,
            "last_delivery_outcome": None,
            "last_delivery_reason": None,
    ...
    def fake_post_json(url: str, payload: dict[str, object], headers: dict[str, str], timeout_s: int) -> dict[str, object]:
        ...
        if url.endswith(f"/v1/projects/{project_id}/workers/find-or-resume"):
            raise dispatch.HttpJsonError(
                (
                    "POST /v1/projects/prj_runtime_dependency/workers/find-or-resume "
                    "failed: HTTP 409 (runtime_dependency_missing): "
                    "GlassHive could not start the selected worker because `codex` is unavailable."
                ),
                status=409,
                method="POST",
                path=f"/v1/projects/{project_id}/workers/find-or-resume",
                payload={
                    "status": "blocked",
                    "detail": "GlassHive could not start the selected worker because `codex` is unavailable.",
                    "failure_class": "runtime_dependency_missing",
                    "failure_retryable": 0,
                    "failure_recommended_recovery": "Use a configured managed dependency.",
                },
                reason="blocked",
                detail="GlassHive could not start the selected worker because `codex` is unavailable.",
                failure_class="runtime_dependency_missing",
        ...
        if url.endswith("/assign"):
            raise AssertionError("assign must not run when find-or-resume is blocked")
        raise AssertionError(url)
...
def test_glasshive_runtime_dependency_missing_recovers_to_docker_when_safe(
    ...
    def fake_post_json(url: str, payload: dict[str, object], headers: dict[str, str], timeout_s: int) -> dict[str, object]:
        if url.endswith("/v1/projects"):
            assert payload["default_worker_profile"] == "codex-cli"
            return {"project_id": "prj_recovery"}
        if url.endswith("/workers/find-or-resume"):
            mode = str(payload["execution_mode"])
            find_modes.append(mode)
            if mode == "host":
                raise dispatch.HttpJsonError(
                    "POST /v1/projects/prj_recovery/workers/find-or-resume failed: HTTP 409 (runtime_dependency_missing): missing host runtime",
                    status=409,
                    method="POST",
                    path="/v1/projects/prj_recovery/workers/find-or-resume",
                    payload={
                        "status": "blocked",
                        "detail": "missing host runtime",
                        "failure_class": "runtime_dependency_missing",
                        "failure_retryable": 0,
                    },
                    reason="blocked",
    ...
    assert result["delivery"]["outcome"] == "queued"
    assert result["delivery"]["reason"] == "glasshive_runtime_recovered_run_queued"
    assert find_modes == ["host", "docker"]
    channel = result["delivery"]["channels"]["workbench"]
    assert channel["runtime_recovery"]["from_execution_mode"] == "host"
    assert channel["runtime_recovery"]["to_execution_mode"] == "docker"
    run = storage.list_scheduled_prompt_runs(definition_id="def-1")[0]
    assert run["status"] == "queued"
    assert "recovery to docker" in run["result_summary"]
    ...
    storage.create_scheduled_prompt_run(
        {
            ...
            "result_summary": (
                f"FINAL REPORT at {synthetic_home_path('private', 'path')} with mongodb://127.0.0.1:27017/db "
            ...
            "callback_payload_json": json.dumps(
                {
                    "event": "run.completed",
                    "message": (
                        f"FINAL REPORT with {synthetic_home_path('private', 'path')}, "
