...
def test_rag_compose_caps_local_resource_defaults() -> None:
    ...
    assert 'max-size: "${VIVENTIUM_RAG_LOG_MAX_SIZE:-5m}"' in compose_text
    assert 'max-file: "${VIVENTIUM_RAG_LOG_MAX_FILE:-3}"' in compose_text
    assert '"127.0.0.1:${VIVENTIUM_RAG_VECTORDB_HOST_PORT:-5433}:5432"' in compose_text
    assert '"127.0.0.1:${RAG_PORT:-8000}:${RAG_PORT:-8000}"' in compose_text
