...
def test_compile_config_preserves_compiler_failure_status() -> None:
    ...
    assert 'compiler_args+=("$@")' in compile_block
    assert '"${compiler_args[@]}" || compile_status=$?' in compile_block
    assert 'if [[ "$compile_status" -ne 0 ]]; then' in compile_block
    assert 'return "$compile_status"' in compile_block


def test_release_identity_forwarding_includes_live_owner_state() -> None:
    source = BIN_VIVENTIUM.read_text(encoding="utf-8")
    compile_block = source[
        source.index("compile_config() {") : source.index(
            "sync_memory_hardening_schedule() {"
        )
    ]

    assert 'runtime_owner_state="$(stack_owner_state_file)"' in compile_block
    assert 'compiler_args+=(--installed-root "$installed_root")' in compile_block
    assert 'compiler_args+=(--runtime-owner-state "$runtime_owner_state")' in compile_block
    assert 'gate_args+=(--installed-root "$installed_root")' in compile_block
    assert 'gate_args+=(--runtime-owner-state "$runtime_owner_state")' in compile_block


def test_stack_owner_state_records_live_process_identity() -> None:
    source = BIN_VIVENTIUM.read_text(encoding="utf-8")
    owner_block = source[
        source.index("write_stack_owner_state() {") : source.index(
            "resolve_release_installed_root() {"
        )
    ]

    assert '"ownerPid"' in owner_block
    assert '"configFile"' in owner_block
    assert '"runtimeDir"' in owner_block
    assert '"componentsLockFile"' in owner_block
    assert '"ownerLaunchMode"' in owner_block
    assert '"ownerExecutablePath"' in owner_block
    assert '"ownerProcessCwd"' in owner_block
    assert '"ownerProcessStartedAt"' in owner_block
    assert '"ownerProcessCommand"' in owner_block
    assert '"ownerBindingSha256"' in owner_block
...
def test_dev_env_offsets_local_glasshive_surfaces_without_rewriting_remote_urls(
    ...
    config["integrations"]["glasshive"] = {
        ...
        "operator_base_url": "http://localhost:8780",
        "provider": {"enabled": True, "base_url": "http://127.0.0.1:8766/v1"},
    }
    ...
    dev_config = yaml.safe_load(
        (app_support / "dev-envs" / "qa" / "config.yaml").read_text(encoding="utf-8")
    )
    glasshive = dev_config["integrations"]["glasshive"]
    assert glasshive["provider"]["base_url"] == "http://127.0.0.1:13766/v1"
    assert glasshive["mcp_url"] == "http://127.0.0.1:13767/mcp"
    assert glasshive["operator_base_url"] == "http://localhost:13780"

    config["integrations"]["glasshive"] = {
        "enabled": True,
        "mcp_url": "https://glasshive.example.com:8767/mcp",
        "operator_base_url": "https://glasshive.example.com:8780",
        "provider": {
            "enabled": True,
            "base_url": "https://glasshive.example.com:8766/v1",
        },
    }
    config_path.write_text(yaml.safe_dump(config, sort_keys=False), encoding="utf-8")
    subprocess.run(
    ...
    remote = yaml.safe_load(
        (app_support / "dev-envs" / "remote" / "config.yaml").read_text(encoding="utf-8")
    )["integrations"]["glasshive"]
    assert remote["provider"]["base_url"] == "https://glasshive.example.com:8766/v1"
    assert remote["mcp_url"] == "https://glasshive.example.com:8767/mcp"
    assert remote["operator_base_url"] == "https://glasshive.example.com:8780"

    config["integrations"]["glasshive"] = {
        "enabled": True,
        "mcp_url": "http://127.0.0.1:8877/mcp",
        "operator_base_url": "http://127.0.0.1:8875",
        "provider": {"enabled": True, "base_url": "http://127.0.0.1:8876/v1"},
    ...
    enterprise = yaml.safe_load(
        (app_support / "dev-envs" / "enterprise" / "config.yaml").read_text(encoding="utf-8")
    )["integrations"]["glasshive"]
    assert enterprise["provider"]["base_url"] == "http://127.0.0.1:8876/v1"
    assert enterprise["mcp_url"] == "http://127.0.0.1:8877/mcp"
    assert enterprise["operator_base_url"] == "http://127.0.0.1:8875"
...
def test_dev_env_stop_removes_only_its_exact_parallel_work_containers(
    tmp_path: Path,
) -> None:
    source = STACK_LAUNCHER.read_text(encoding="utf-8")
    assert "parallel_work_runtime_instance() {" in source
    assert "stop_parallel_work_runtime_containers() {" in source

    instance_start = source.index("parallel_work_runtime_instance() {")
    stop_helper_start = source.index("stop_parallel_work_runtime_containers() {")
    resolve_python_start = source.index("resolve_python_bin() {", stop_helper_start)
    remove_project_start = source.index("remove_compose_project_containers() {")
    remove_service_start = source.index(
        "remove_compose_service_containers() {",
        remove_project_start,
    )
    instance_helper = source[instance_start:stop_helper_start]
    stop_helper = source[stop_helper_start:resolve_python_start]
    remove_project_helper = source[remove_project_start:remove_service_start]

    stop_block = source[
        source.index("stop_running_services() {") : source.index(
            "cleanup_stale_containers() {"
        )
    ]
    glasshive_stop = stop_block.split(
        'if [[ -d "$GLASSHIVE_RUNTIME_DIR"',
        1,
    )[1].split("# === VIVENTIUM START ===", 1)[0]
    assert glasshive_stop.strip().endswith("stop_parallel_work_runtime_containers")

    proxy_start = source[
        source.index("start_parallel_work_proxy_substrate() {") : source.index(
            "start_glasshive() {"
        )
    ]
    assert 'instance="$(parallel_work_runtime_instance)"' in proxy_start
    assert "tr -cs 'a-z0-9_.-' '-'" not in proxy_start

    def run_cleanup(
        capture_file: Path,
        *,
        service: str = "parallel-work-redis",
        stack: str = "viventium_v0_4",
        profile: str = "isolated",
        contract: str = "parallel-work-redis-v1",
        redis_id: str = "redis-id",
...
docker() {{
  ...
  case "$1" in
    ps)
      if [[ "$*" == *"label=com.docker.compose.project=viventium-parallel-proxy-rollback-final-20260827"* ]]; then
        printf 'provider-id\\nbroker-id\\n'
      elif [[ "$*" == *"name=^/viventium-parallel-redis-isolated-rollback-final-20260827$"* ]]; then
        printf '%s\\n' "$FAKE_REDIS_ID"
      else
        printf 'canonical-id\\nforeign-id\\nshared-id\\n'
      fi
    ...
    inspect)
      case "$3" in
        *viventium.service*) printf '%s\\n' "$FAKE_REDIS_SERVICE" ;;
        *viventium.stack*) printf '%s\\n' "$FAKE_REDIS_STACK" ;;
        *viventium.profile*) printf '%s\\n' "$FAKE_REDIS_PROFILE" ;;
        *viventium.contract*) printf '%s\\n' "$FAKE_REDIS_CONTRACT" ;;
        *) return 97 ;;
      esac
      ;;
    rm) return 0 ;;
...
SKIP_DOCKER=false
VIVENTIUM_DEV_ENV_INSTANCE_ID='Rollback Final / 20260827'
VIVENTIUM_RUNTIME_PROFILE=isolated
PARALLEL_REDIS_CONTAINER_NAME=viventium-parallel-redis-isolated-rollback-final-20260827
        ...
        return subprocess.run(
            ...
            env={
                **os.environ,
                ...
                "FAKE_REDIS_SERVICE": service,
                "FAKE_REDIS_STACK": stack,
                "FAKE_REDIS_PROFILE": profile,
                "FAKE_REDIS_CONTRACT": contract,
                "FAKE_REDIS_ID": redis_id,
            },
            text=True,
            timeout=5,
        )

    exact_capture = tmp_path / "exact-docker-calls.txt"
    exact = run_cleanup(exact_capture)
    assert exact.returncode == 0, exact.stderr
    exact_calls = exact_capture.read_text(encoding="utf-8")
    assert "rm -f provider-id broker-id" in exact_calls
    assert "rm -f redis-id" in exact_calls
    assert "volume" not in exact_calls
    for untouched in ("canonical-id", "foreign-id", "shared-id"):
        assert untouched not in exact_calls

    for label, override in (
        ("service", {"service": "foreign-service"}),
        ("stack", {"stack": "foreign-stack"}),
        ("profile", {"profile": "compat"}),
        ("contract", {"contract": "foreign-contract"}),
    ):
        foreign_capture = tmp_path / f"foreign-{label}-docker-calls.txt"
        foreign = run_cleanup(
            foreign_capture,
            redis_id=f"foreign-{label}-redis-id",
            **override,
        )
        assert foreign.returncode == 0, foreign.stderr
        foreign_calls = foreign_capture.read_text(encoding="utf-8")
        assert "rm -f provider-id broker-id" in foreign_calls
        assert f"rm -f foreign-{label}-redis-id" not in foreign_calls
        assert "volume" not in foreign_calls


def test_dev_env_stop_boots_out_its_exact_telegram_job_before_killing_the_pid() -> None:
    source = STACK_LAUNCHER.read_text(encoding="utf-8")
    stop_block = source[source.index("stop_running_services() {") : source.index("cleanup_stale_containers() {")]
    telegram_stop = stop_block.split("# Telegram bot", 1)[1].split(
        "stop_telegram_local_bot_api", 1
    )[0]

    bootout_position = telegram_stop.index("stop_telegram_launchctl_job")
    pid_kill_position = telegram_stop.index("stop_owned_telegram_poller")

    assert bootout_position < pid_kill_position
    assert "runtime_allows_workspace_wide_process_sweep" not in telegram_stop[:bootout_position]
    assert 'kill_pids "$telegram_pid"' not in telegram_stop
    stop_owned = source[
        source.index("stop_owned_telegram_poller() {") :
        source.index("telegram_deferred_pid_is_running() {")
    ]
    assert '"$TELEGRAM_POLLER_HANDOFF_HELPER" stop-owned' in stop_owned
    assert '--state-dir "$TELEGRAM_POLLER_STATE_DIR"' in stop_owned
    assert '--pid-file "$TELEGRAM_BOT_PID_FILE"' in stop_owned
    assert 'VIVENTIUM_DEV_ENV_INSTANCE_ID:-' in source
    assert 'ai.viventium.telegram-bot.${telegram_launchctl_instance}' in source


def test_glasshive_readiness_uses_public_health_endpoint() -> None:
    source = STACK_LAUNCHER.read_text(encoding="utf-8")
    body = source[
        source.index("glasshive_stack_ready() {") : source.index("start_glasshive() {")
    ]

    assert 'curl -fsS "http://127.0.0.1:${GLASSHIVE_UI_PORT}/health"' in body
    assert 'curl -fsS "http://127.0.0.1:${GLASSHIVE_UI_PORT}/"' not in body


def test_glasshive_ui_receives_the_selected_runtime_endpoint() -> None:
    source = STACK_LAUNCHER.read_text(encoding="utf-8")

    port_assignment = source.index('GLASSHIVE_RUNTIME_PORT="${GLASSHIVE_RUNTIME_PORT:-8766}"')
    ui_launch = source.index(
        'uv run uvicorn glass_drive_ui.server:app --host 127.0.0.1 --port "$GLASSHIVE_UI_PORT"'
    )
    runtime_endpoint_export = source.index(
        'export GLASSHIVE_RUNTIME_BASE_URL="${GLASSHIVE_RUNTIME_BASE_URL:-http://127.0.0.1:${GLASSHIVE_RUNTIME_PORT}}"'
    )
    mcp_runtime_endpoint_binding = source.index(
        'WPR_MCP_BASE_URL="http://127.0.0.1:${GLASSHIVE_RUNTIME_PORT}"'
    )

    assert port_assignment < runtime_endpoint_export < mcp_runtime_endpoint_binding < ui_launch


def test_local_glasshive_mcp_child_overrides_stale_ambient_runtime_endpoint() -> None:
    source = STACK_LAUNCHER.read_text(encoding="utf-8")
    start_block = source[
        source.index("start_glasshive() {") : source.index("start_ms365_mcp() {")
    ]

    assert (
        'WPR_MCP_BASE_URL="http://127.0.0.1:${GLASSHIVE_RUNTIME_PORT}" \\\n'
        '    uv run python -m workers_projects_runtime.mcp_server'
    ) in start_block
...
def test_dev_env_offsets_default_app_facing_and_local_runtime_ports_during_compile(
    tmp_path: Path,
) -> None:
    config = minimal_config()
    # Keep unrelated compiler assertions cross-architecture.
    ...
    assert "VIVENTIUM_LOCAL_MONGO_PORT=28117" in env_text
    assert "VIVENTIUM_LOCAL_MEILI_PORT=8700" in env_text
    assert "LIVEKIT_HTTP_PORT=8888" in env_text
    assert "LIVEKIT_TCP_PORT=8889" in env_text
    assert "LIVEKIT_UDP_PORT=8890" in env_text
    assert "VIVENTIUM_CODE_INTERPRETER_PORT=9101" in env_text
    assert "VIVENTIUM_SKYVERN_API_PORT=9200" in env_text
    assert "VIVENTIUM_SKYVERN_UI_PORT=9280" in env_text
    assert "VIVENTIUM_PROMPT_WORKBENCH_PORT=8781" in env_text
    assert "VIVENTIUM_RAG_API_PORT=8110" in env_text
    assert "VIVENTIUM_RAG_VECTORDB_HOST_PORT=5433" in env_text
    assert "VIVENTIUM_GOOGLE_MCP_PORT=8111" in env_text
