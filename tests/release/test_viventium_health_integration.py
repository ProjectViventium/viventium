...
HEALTH_TOOL_IDS = {
    ...
    "health_read_image_mcp_viventium-health",
    "health_read_record_mcp_viventium-health",
}
HEALTH_TOOL_NAMES = {tool_id.rsplit("_mcp_", 1)[0] for tool_id in HEALTH_TOOL_IDS}
...
def expected_health_server() -> dict:
    return {
        ...
        "viventiumGlassHive": {
            ...
            "toolPolicies": {
                tool_name: {"access": "content_read"}
                for tool_name in sorted(HEALTH_TOOL_NAMES)
            },
        },
        "serverInstructions": (
...
def test_compiler_omits_viventium_health_when_the_integration_is_disabled() -> None:
    servers = config_compiler.build_mcp_servers(
        {"integrations": {"health": {"enabled": False}}},
        {"lc_api_port": 3180},
        "agent_viventium_main_95aeb3",
    )

    assert HEALTH_SERVER_NAME not in servers
