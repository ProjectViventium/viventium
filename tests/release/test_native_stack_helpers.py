...
def test_start_livekit_restores_missing_pid_for_exact_listener(tmp_path: Path) -> None:
    script_text = NATIVE_STACK_PATH.read_text(encoding="utf-8")
    defs = "".join(
        extract_shell_function(script_text, name) for name in ("write_pid", "start_livekit")
    )
    ...
    completed = subprocess.run(
        [
            ...
            (
                ...
                "port_listening() { return 0; }\n"
                "resolve_unique_listener_pid() { printf '333\\n'; }\n"
                "managed_livekit_listener_pid() { printf '333\\n'; }\n"
                "livekit_command_matches_expected() { [[ \"$1\" == '333' ]]; }\n"
                "livekit_meta_matches_expected() { return 0; }\n"
                f"{defs}"
                "start_livekit\n"
                "printf 'pid=%s\\n' \"$(tr -d '[:space:]' <\"$LIVEKIT_PID_FILE\")\"\n"
            ),
        ],
        cwd=REPO_ROOT,
        check=True,
    ...
    assert "pid=333" in completed.stdout
    assert "adopted listener PID 333" in completed.stdout
...
def test_native_stop_validates_each_runtime_process_before_killing() -> None:
    ...
    assert 'stop_pid_file_if_matches "$MEILI_PID_FILE" "Meilisearch" meili_process_matches_expected' in stop_case
    assert 'stop_pid_file_if_matches "$MONGO_PID_FILE" "MongoDB" mongo_process_matches_expected' in stop_case
    ...
    completed = subprocess.run(
        [
            ...
            (
                ...
                "livekit_command_matches_expected() { return 0; }\n"
