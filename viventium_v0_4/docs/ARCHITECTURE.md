...
stable/base/MCP instructions. `all_agents` covers eligible persona-bearing Main, handoff, speaking
Phase B, and direct GlassHive worker paths. Specialist background cortices remain independent and
receive only structured Feeling telemetry when their own contract needs it; they never receive the
embodiment capsule. `conscious_agent` limits the capsule to the conscious speaking path.
...
3. Phase A activation detection runs in the mode-specific fast window: 1300 ms for text and 690 ms
   for voice by shipped default. The shared 2000 ms value is only the fallback when a mode-specific
   value is unset. Timed-out or partial detection may continue in the non-blocking 6000 ms late-
   recovery window without delaying Main; only newly recovered cortices execute.
...
9. A single follow-up message is adjudicated (LLM-driven) if insights exist. If newer user input
   arrived, the adjudicator receives the newer visible exchange and returns `{NTA}` unless the
   insight remains useful in the current conversation.
...
- Newer user input does not cause an unconditional timestamp suppression. It is included in the
  follow-up adjudication context; the adjudicator returns `{NTA}` unless the insight remains useful.
...
LibreChat Core owns provider-neutral `MainContextSnapshotV1`, accepted logical-turn state,
`ReplyContextV1`, and `RecurrenceStateV1`. GlassHive owns bounded native execution and persists
`ReplayDecisionV1` plus accepted/reserved stable keys. Per-turn state cannot rotate stable authority,
and provider fallback cannot rebuild an independent history. The parent requirement and acceptance
owners are `docs/requirements_and_learnings/56_Main_Continuity_Kernel.md` and
`qa/main-continuity/`.
