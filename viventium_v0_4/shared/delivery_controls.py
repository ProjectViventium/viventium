...
def parse_delivery_controls(
    text: Optional[str], *, max_message_breaks: Any = DEFAULT_MAX_MESSAGE_BREAKS
) -> DeliveryControls:
    ...
    try:
        numeric_max = float(max_message_breaks)
        safe_max_breaks = (
            max(0, int(numeric_max))
            if math.isfinite(numeric_max)
            else DEFAULT_MAX_MESSAGE_BREAKS
        )
    except (TypeError, ValueError, OverflowError):
        safe_max_breaks = DEFAULT_MAX_MESSAGE_BREAKS
...
def strip_incomplete_control_suffix(text: Optional[str]) -> str:
    ...
    incomplete = any(
        1 <= len(compact) < len(token) and token.startswith(compact)
