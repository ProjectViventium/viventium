import type {
  ...
  ScheduledPromptPeripheryArtifactDetail,
  ScheduledPromptPeripheryIndex,
  ScheduledPromptPeripherySnapshot,
...
async function responseMessage(response: Response) {
  ...
  try {
    ...
    if (detail && typeof detail === "object" && "message" in detail) {
      return String(
        (detail as { message?: unknown }).message || response.statusText,
      );
    }
  } catch {
    return text;
...
export function getPrompts() {
  return api<{ prompts: PromptRow[]; flow: FlowGraph; evalBank: EvalBank }>(
    "/api/prompts",
  );
}
...
export function getNightlyScheduledPromptTemplate() {
  return api<ScheduledPromptTemplate>(
    "/api/scheduled-prompts/templates/nightly-subconscious",
  );
}
...
export function createScheduledPrompt(
  payload: Partial<ScheduledPrompt> & { title: string; promptText: string },
) {
...
export function updateScheduledPrompt(
  id: string,
  payload: Partial<ScheduledPrompt>,
) {
  return api<ScheduledPrompt>(
    `/api/scheduled-prompts/${encodeURIComponent(id)}`,
    {
      method: "PATCH",
      body: JSON.stringify(payload),
    },
...
export function manualRunScheduledPrompt(
  id: string,
  confirmUserLevelDelivery = false,
) {
  return api<{ run?: ScheduledPromptRun; dispatch?: unknown }>(
    `/api/scheduled-prompts/${encodeURIComponent(id)}/manual-runs`,
    {
      method: "POST",
...
export function getScheduledPromptRuns(id: string) {
  return api<{ runs: ScheduledPromptRun[] }>(
    `/api/scheduled-prompts/${encodeURIComponent(id)}/runs`,
  );
}
...
export function getScheduledPromptPeripheryArtifacts(id: string) {
  return api<{
    ...
    index: ScheduledPromptPeripheryIndex;
    contract: string;
  }>(`/api/scheduled-prompts/${encodeURIComponent(id)}/periphery-artifacts`);
}

export function getScheduledPromptPeripheryArtifact(
  id: string,
  artifactId: string,
) {
  return api<ScheduledPromptPeripheryArtifactDetail>(
    `/api/scheduled-prompts/${encodeURIComponent(id)}/periphery-artifacts/${encodeURIComponent(artifactId)}`,
  );
}

export function getScheduledPromptPeripherySnapshot(id: string) {
  return api<{ snapshot: ScheduledPromptPeripherySnapshot; contract: string }>(
    `/api/scheduled-prompts/${encodeURIComponent(id)}/periphery-snapshot`,
  );
}

export function refreshScheduledPromptPeripherySnapshot(id: string) {
  return api<{ snapshot: ScheduledPromptPeripherySnapshot; contract: string }>(
    `/api/scheduled-prompts/${encodeURIComponent(id)}/periphery-snapshot`,
    { method: "POST" },
  );
...
export function applyScheduledPromptMemoryProposal(
  id: string,
  proposalId: string,
  apply = false,
) {
...
export function getPromptRevision(id: string, revision: string) {
  return api<PromptRevision>(
    `/api/prompts/${encodeURIComponent(id)}/revisions/${encodeURIComponent(revision)}`,
  );
}

export function getPromptWorkbenchContext(id: string) {
  return api<PromptWorkbenchContext>(
    `/api/prompts/${encodeURIComponent(id)}/workbench-context`,
  );
}
...
export function pullLive() {
  return api<unknown>("/api/sync/pull-live", {
    method: "POST",
...
export function applyDraft(id: string, idempotencyToken: string) {
  ...
  }>(`/api/drafts/${encodeURIComponent(id)}/apply`, {
    method: "POST",
...
export function discardDraft(id: string) {
  return api<{ id: string; status: string; targetPath: string }>(
...
export function createEvalCaseDraft(options: {
  familyId: string;
  caseId: string;
  ...
  return api<DraftRecord>("/api/evals/case-draft", {
    ...
    body: JSON.stringify({
      ...
      reason:
        options.reason ||
        `Workbench eval edit for ${options.familyId}/${options.caseId}`,
    }),
  });
}
