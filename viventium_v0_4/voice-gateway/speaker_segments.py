...
def shared_microphone_state_applies_to_track(
    session_state: Any, track_sid: str, participant_identity: str = ""
) -> bool:
    """Restore shared-mic state by stable participant identity across track replacement."""
    if not isinstance(session_state, dict) or (
        session_state.get("attributionState") != "shared_mic_unverified"
    ):
        return False
    shared_participant_identities = session_state.get("sharedParticipantIdentities")
    if not isinstance(shared_participant_identities, list):
        # Track SIDs are publication-scoped and change on reconnect. A legacy track-only state
        # therefore cannot safely prove that a replacement track is unrelated.
        return True
    normalized = {
        value.strip()
    ...
    normalized_participant_identity = str(participant_identity or "").strip()
    if not normalized or not normalized_participant_identity:
        return True
    return normalized_participant_identity in normalized
...
class SpeakerSegmentTracker:
                    ...
                    self._pending_session_states.append(
                        {
                            ...
                            **(
                                {
                                    ...
                                    "sharedTrackSids": [self._track_sid],
                                }
                                if self._track_sid
                                else {}
                            ),
                            **(
                                {
                                    "sourceParticipantIdentity": self._participant_identity,
                                    "sharedParticipantIdentities": [self._participant_identity],
                                }
    ...
    def requeue_session_state_changes(self, states: list[dict[str, Any]]) -> None:
        """Restore unacknowledged state so transport retries cannot lose the tombstone."""
        restored = [deepcopy(item) for item in states if isinstance(item, dict)]
        if restored:
            self._pending_session_states = [*restored, *self._pending_session_states]
