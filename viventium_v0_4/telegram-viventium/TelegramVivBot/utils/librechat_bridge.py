...
class LibreChatBridge:
    ...
    def _set_active_stream(self, chat_id: str, stream_id: str) -> None:
        ...
        if previous and previous != stream_id:
            ...
            previous_followup = self._followup_task_by_stream.get(previous)
            keep_previous_followup = bool(
                previous_followup
                and not previous_followup.done()
                and not self._has_followup_sent(previous)
            )
            self._trace(
            ...
            if not keep_previous_glasshive and not keep_previous_followup:
                self._cancel_followup_task(previous)
                self._pending_followups.pop(previous, None)
                self._pending_stream_errors.pop(previous, None)
