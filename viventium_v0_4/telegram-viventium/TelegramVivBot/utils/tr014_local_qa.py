"""One-shot installed local-QA timing control for Telegram case TR-014."""
...
TR014_CASE_ID = "TR-014"
TR014_LOCAL_QA_MODE = "tr-014"
...
class ArmReceipt:
    ...
    def case_id(self) -> str:
        return TR014_CASE_ID

    @property
    def delay_ms(self) -> int:
        return TR014_DELAY_MS
...
class TR014ControlError(RuntimeError):
...
class _PrivateArgumentParser(argparse.ArgumentParser):
    def error(self, _message: str) -> None:
        raise TR014ControlError("invalid_arguments")
...
def _control_root(state_dir: Path | None = None) -> Path:
    return Path(state_dir or _default_state_dir()) / "local-qa" / "tr-014"


def _assert_private_directory(path: Path) -> None:
    if path.is_symlink():
        raise RuntimeError("TR-014 local-QA state directory cannot be a symlink")
    path.mkdir(parents=True, exist_ok=True, mode=0o700)
    if not path.is_dir() or path.stat().st_uid != os.getuid():
        raise RuntimeError("TR-014 local-QA state directory is not owner-controlled")
    os.chmod(path, 0o700)
...
def _atomic_json(path: Path, payload: dict, *, mode: int) -> None:
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                raise OSError("Could not persist TR-014 local-QA artifact")
            view = view[written:]
        os.fsync(descriptor)
...
def _strict_case_token(case_token: object) -> str:
    ...
    if (
        ...
        raise ValueError("invalid TR-014 case token")
    return token


def _strict_case_id(case_id: object) -> str:
    if str(case_id or "") != TR014_CASE_ID:
        raise ValueError("TR-014 requires the canonical case ID")
    return TR014_CASE_ID
...
def _strict_session_ref(case_token: str, session_ref: object) -> str:
    ...
    if normalized != expected:
        raise ValueError("TR-014 session reference does not match the canonical token")
    return normalized
    ...
    for expected_index, path in enumerate(
        ...
        try:
            ...
            if (
                ...
                or item.get("case_id") != TR014_CASE_ID
            ...
            raise ValueError("TR-014 audit ledger integrity check failed") from exc
        records.append(item)
...
def _audit(
    ...
    *,
    ...
    payload = {
        ...
        "outcome": outcome,
        "reason": reason,
        "configured_delay_ms": TR014_DELAY_MS,
...
def arm_tr014_control(
    *,
    ...
    """Create one private, exact, short-lived TR-014 control artifact."""
    ...
    plan = {
        ...
        "local_qa_mode": TR014_LOCAL_QA_MODE,
        "case_token_sha256": digest,
        "session_ref": normalized_session_ref,
        "created_at_ms": created_at_ms,
        "expires_at_ms": expires_at_ms,
        "delay_ms": TR014_DELAY_MS,
        "event": {
    ...
    with _locked(root):
        if any((root / "armed").glob("*.json")):
            raise TR014ControlError("control_already_armed")
...
def _load_plan(path: Path) -> dict:
    ...
    if (
        ...
        or raw.get("case_id") != TR014_CASE_ID
        or raw.get("local_qa_mode") != TR014_LOCAL_QA_MODE
        or raw.get("delay_ms") != TR014_DELAY_MS
...
async def maybe_delay_tr014_core_ingestion(
    ...
    *,
    ...
    try:
        await delay(TR014_DELAY_MS / 1000)
    except BaseException:
        with _locked(root):
            _audit(
                root,
        ...
        raise
    return DelayResult(True, "applied", TR014_DELAY_MS, claimed_digest[:16])


def cleanup_tr014_control(
    *,
    case_token: str,
    session_ref: str,
    case_id: str = TR014_CASE_ID,
    state_dir: Path | None = None,
) -> bool:
...
def _parser() -> argparse.ArgumentParser:
    parser = _PrivateArgumentParser(
        prog="tr014-local-qa",
        description="Manage the one-shot TR-014 local-QA control",
    )
    ...
    cleanup.add_argument("--case-id", required=True, choices=[TR014_CASE_ID])
    ...
    audit.add_argument("--case-id", required=True, choices=[TR014_CASE_ID])
...
def _read_bounded_private_stdin(
    *, max_bytes: int, missing_code: str, invalid_code: str
    ...
    if len(encoded) > max_bytes:
        raise TR014ControlError(invalid_code)
    try:
        text = encoded.decode("utf-8")
    except UnicodeDecodeError as exc:
        raise TR014ControlError(invalid_code) from exc
    if not text.strip():
        raise TR014ControlError(missing_code)
    return text
...
def _read_private_arm_envelope() -> dict:
    ...
    try:
        ...
        raise TR014ControlError("invalid_private_arm_envelope") from exc
    return envelope
...
def _read_case_token_from_stdin() -> str:
    ...
    if not token:
        raise TR014ControlError("case_token_required_on_stdin")
    try:
        return _strict_case_token(token)
    except ValueError as exc:
        raise TR014ControlError("invalid_case_token") from exc
...
def main(argv=None) -> int:
    try:
        ...
        return 0
    except TR014ControlError as exc:
        error = exc.code
    except ValueError:
    ...
    print(
        json.dumps({"case_id": TR014_CASE_ID, "error": error}, sort_keys=True),
        file=sys.stderr,
    )
    return 2
