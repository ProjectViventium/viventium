...
class OrchestrationSnapshot:
    ...
    release_gate_blockers: tuple[str, ...]
...
def parse_snapshot(
    ...
    *,
    ...
    raw_release_gate = preference_payload.get("releaseGate")
    release_gate_label = ""
    release_gate_blockers: list[str] = []
    if isinstance(raw_release_gate, dict):
        release_gate_label = _safe_text(raw_release_gate.get("label"), limit=80)
        raw_blockers = raw_release_gate.get("blockers")
        if isinstance(raw_blockers, list):
            for value in raw_blockers[:50]:
                blocker = _safe_text(value, limit=160)
                if blocker and blocker not in release_gate_blockers:
                    release_gate_blockers.append(blocker)
    ...
    return OrchestrationSnapshot(
        ...
        release_gate_label=release_gate_label,
        release_gate_blockers=tuple(release_gate_blockers),
...
def format_active_work(snapshot: OrchestrationSnapshot) -> str:
    ...
    if snapshot.release_gate_label:
        lines.append(snapshot.release_gate_label)
        if snapshot.release_gate_blockers:
            lines.append(", ".join(snapshot.release_gate_blockers))
...
def format_parallel_work_settings(snapshot: OrchestrationSnapshot) -> str:
    ...
    if snapshot.release_gate_label:
        lines.extend(["", snapshot.release_gate_label])
        if snapshot.release_gate_blockers:
            lines.append(", ".join(snapshot.release_gate_blockers))
