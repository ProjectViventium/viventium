...
| --- | --- | --- | --- | --- | --- |
| `TGMEDIA-001` | Telegram media downloads are retried, stored, and surfaced with accurate user-facing failure copy. | User-visible behavior matches source, docs, persisted state, and logs | Telegram media, download logs, STT/attachment flow | `tests/release/test_telegram_media_prereqs.py` plus user-grade QA when visible | NOT RUN (cataloged 2026-05-17; next feature run required) |
| `TGMEDIA-002` | Public QA evidence is sanitized and reproducible | A PR reviewer can verify the behavior without private/local data | QA report, git diff, logs summary, generated artifacts | Public-safety scan plus relevant release tests | NOT RUN (cataloged 2026-05-17; next feature run required) |
...
| --- | --- | --- | --- | --- | --- | --- |
| `TGMEDIA-UC-001` | On Telegram media, download logs, STT/attachment flow, verify that telegram media downloads are retried, stored, and surfaced with accurate user-facing failure copy. | owning requirement for `TGMEDIA-001` / `TGMEDIA-001` | Telegram media, download logs, STT/attachment flow | Source, owning requirement doc, case steps, logs, DB/state, generated config, and shipped artifact evidence that apply to TGMEDIA-001. | User-visible behavior matches source, docs, persisted state, and logs | NOT RUN (cataloged 2026-05-18; next feature run required) |
| `TGMEDIA-UC-002` | On QA report, git diff, logs summary, generated artifacts, create or review the public QA evidence record with setup/auth/config, empty-state, degraded-dependency, and privacy checks. | owning requirement for `TGMEDIA-002` / `TGMEDIA-002` | QA report, git diff, logs summary, generated artifacts | Source, owning requirement doc, case steps, logs, DB/state, generated config, and shipped artifact evidence that apply to TGMEDIA-002. | The user sees an honest setup, retry, or degraded-state result for TGMEDIA-002; no fake success is accepted. | NOT RUN (cataloged 2026-05-18; next feature run required) |
