...
| --- | --- | --- | --- | --- | --- |
| `LISTEN-001` | Passive/listen-only surfaces stay silent unless directly addressed or safety/time-sensitive conditions require output. | User-visible behavior matches source, docs, persisted state, and logs | voice/listen-only UI, transcript, final visible output | `tests/release/test_prompt_architecture_eval_harness.py` plus user-grade QA when visible | NOT YET RUN (cataloged 2026-05-17; next feature run required) |
...
- Requirement: Passive/listen-only surfaces stay silent unless directly addressed or safety/time-sensitive conditions require output.
...
| --- | --- | --- | --- | --- | --- | --- |
| `LISTEN-UC-001` | On voice/listen-only UI, transcript, final visible output, verify that passive/listen-only surfaces stay silent unless directly addressed or safety/time-sensitive conditions require output. | owning requirement for `LISTEN-001` / `LISTEN-001` | voice/listen-only UI, transcript, final visible output | Source, owning requirement doc, case steps, logs, DB/state, generated config, and shipped artifact evidence that apply to LISTEN-001. | User-visible behavior matches source, docs, persisted state, and logs | NOT YET RUN (cataloged 2026-05-18; next feature run required) |
