...
| Deep Memory Search | `ANTI-010`, `ACT-46` | `ANTI-002`, `ANTI-005`, `ACT-46` | Structured `activation.mode: always`; authorized retrieval, Phase B value-gating, and silent empty/unavailable results replace classifier coverage. |
...
- `ACT-46` protects specialist output ownership: Main-only exact wording/shape constraints cannot
  turn a substantive cortex into an echo of the initial answer, and Main's actionable request
  cannot replace a retrieval specialist's assignment. Native input and real tool effects are
  required evidence alongside the reply.
