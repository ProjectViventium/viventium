...
| `BACKGROUND-UC-009` | Open the built-in conscious agent and representative Sol/Terra cortices, inspect their saved model/fallback bags, send synthetic chat prompts, and reload. | `ACT-35` / GPT-5.6 workload routing and fallback contract | Real Agent Builder and browser chat using the Viventium QA account, plus live/source/generated config and logs/DB | Saved primary/fallback/voice fields, generated runtime assignments, response model/log class, visible answer/cards, persistence after refresh, and fallback auth state | Main and deep reasoning run on Sol at their mapped efforts; balanced/latency-sensitive cortices use Opus 5 as the managed fresh-install text fallback; explicit existing-user fallback choices survive upgrade; Grok 4.3 remains voice primary; missing fallback auth is classified honestly. | PARTIAL (previous Opus 4.8 live path passed 2026-07-09; Opus 5 post-upgrade user-path acceptance is required) |
