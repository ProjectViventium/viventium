...
| `ACT-45` | A visible background-cortex result recovered through its configured model fallback says so in the persisted card | PASS-LIVE/AUTOMATED 2026-08-09: completion-payload, normal/resumable SSE, typed content-part, renderer, expansion, and reload regressions passed; a disposable non-admin browser turn then proved activation, failed primary, fallback insight, expanded public disclosure, reload persistence, DB agreement, zero console errors, and exact cleanup ([report](reports/2026-08-09-background-cortex-fallback-disclosure.md)) |
...
| --- | --- | --- | --- | --- | --- | --- |
| `BACKGROUND-UC-001` | Send a synthetic browser chat prompt that should activate background work and inspect the visible answer plus background cards. | `ACT-01`-`ACT-23` / background-card requirements | Real browser chat, expanded background cards, persisted message detail, and backend logs/DB | ACT prompt bank, coverage matrix, visible cards, stored message parts, logs, and dated QA report | The initial answer remains visible, background cards show status/result/error details, and the main answer does not contradict visible runtime cards. | 2026-07-10 PASS for named positive and implicit comfort-avoidance Red Team + Confirmation Bias paths on the dedicated QA account, including execution fallback, expanded cards, Mongo state, and reload; see [report](reports/2026-07-09-activation-routing-model-eval.md) |
| `BACKGROUND-UC-002` | Send synthetic negative-control prompts, auth/provider-blocked prompts, and latest-message-history prompts. | `ACT-01`-`ACT-23` / activation and blocker requirements | Real browser chat plus logs/DB state | ACT prompt bank, classifier/runtime assertions, visible blocker copy, stored message parts, and logs | Non-activation prompts do not create stale cards; provider/auth blocks are classified as blocked evidence, not feature failures or fake successes. | 2026-07-09 PASS for positive setup -> output-only latest-turn negative control; provider availability remains separately PARTIAL; see [report](reports/2026-07-09-activation-routing-model-eval.md) |
...
| `BACKGROUND-UC-008` | Trigger or fixture a visible stream/canonical aggregation mismatch while Phase B has completed background evidence. | `ACT-34` / visible Phase A durability before Phase B | Deterministic runtime fixture plus real browser, Telegram, and voice smoke where available; persisted message state and backend logs | Emitted visible delta, repaired parent text, follow-up decision metadata, absence of `on_cortex_followup`, no deterministic fallback copy | The already-visible Phase A answer survives as parent text, structured cortex parts persist, Phase B records `suppressed` with mismatch-repair reason, and no second contradictory follow-up is delivered. | 2026-06-10 PASS for real browser and Telegram owner-account synthetic runs plus targeted automated regressions. Modern Playground voice smoke was PARTIAL: call/session/TTS/backend suite passed, but post-prompt visible transcript proof did not complete inside the QA window. See `reports/2026-06-10-act34-visible-delta-repair.md` |
| `BACKGROUND-UC-009` | Open the built-in conscious agent and representative Sol/Terra cortices, inspect their saved model/fallback bags, send synthetic chat prompts, and reload. | `ACT-35` / GPT-5.6 workload routing and fallback contract | Real Agent Builder and browser chat using the Viventium QA account, plus live/source/generated config and logs/DB | Saved primary/fallback/voice fields, generated runtime assignments, response model/log class, visible answer/cards, persistence after refresh, and fallback capability state | Main and deep reasoning run on Sol at their mapped efforts; balanced/latency-sensitive cortices run on Terra; all generic Agent text fallback bags show `glasshive-harness / claude-code:opus / high`; the explicit voice route remains independent; unavailable fallback capability is classified honestly. | PARTIAL (2026-08-03; source/compiler/runtime tests pass; installed-runtime browser rerun requires runtime activation; [report](../glasshive-core-provider/reports/2026-08-03-agent-builder-glasshive-fallback.md)) |
...
| `BACKGROUND-UC-011` | Ask for high-EQ reading of consequential subtext and, separately, adversarial stress-testing while the conscious agent has an active Feelings state. | `ACT-44`, `EMO-040`, `PW-042` / conscious-subconscious boundary | Real browser chat with expanded background cards plus Prompt Workbench direct specialist families, refresh, logs, and DB/prompt-frame state | Direct specialist model verdicts, prompt lineage, runtime-context count, `specialist_cortex_independent` telemetry, Phase-B `conscious_synthesis` placement, stored cortex parts, and visible final wording | Emotional Resonance reports only supported consequential subtext and stops cleanly when none exists; Red Team challenges the task without adopting Feelings; neither specialist receives the capsule; the user-visible Phase-B synthesis receives the pinned capsule once. | PASS-AUTOMATED/MODEL 2026-07-15: direct Emotional Resonance 2/2 and Red Team 2/2 semantic evals, 40/40 specialist injection tests, and 195/195 affected Phase-B tests; post-promotion browser/log/DB acceptance is pending ([report](reports/2026-07-15-specialist-cortex-execution.md)) |
| `BACKGROUND-UC-012` | Send a normal browser turn that activates a cortex while its primary provider is unavailable and its configured fallback is healthy. | `ACT-45` / visible background model-fallback provenance | Non-admin browser, expanded cortex card, reload, persisted message, provider/runtime logs | Cortex name/status, fallback disclosure and reason, fallback-produced insight, `fallback_used` / `fallback_reason_class`, console, and cleanup | The user sees the useful fallback-produced cortex result plus an honest expandable disclosure; both survive reload without raw provider detail or fixture residue. | PASS-LIVE 2026-08-09: activation, deliberately unavailable synthetic primary, healthy configured OpenAI fallback insight, expanded disclosure/public class, reload, persistence, zero console errors, and exact two-Agent/conversation/session cleanup all agreed ([report](reports/2026-08-09-background-cortex-fallback-disclosure.md)) |
