...
| `ACT-38` | A Phase A provider that ignores cancellation cannot monopolize the global classifier budget | Non-cooperative promise regression, per-attempt timeout evidence, fallback completion inside the same global budget, and no user-text heuristic |
