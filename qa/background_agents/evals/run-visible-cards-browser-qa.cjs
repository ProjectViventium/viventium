...
function parseArgs(argv) {
  ...
  const args = {
    ...
    out:
      ...
      path.join(REPO_ROOT, 'qa', 'background_agents', 'visible_cards_browser_qa_2026-05-10.md'),
    headless: process.env.VIVENTIUM_QA_HEADLESS !== '0',
    timeoutMs: Number(process.env.VIVENTIUM_QA_TIMEOUT_MS || 180000),
    prompt: process.env.VIVENTIUM_QA_PROMPT || `${DEFAULT_PROMPT} Synthetic QA marker: ${promptMarker}.`,
  };
...
function parseRequiredCortexAgentIdsByName() {
  ...
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('Invalid VIVENTIUM_QA_REQUIRED_CORTEX_AGENT_IDS_JSON: expected object');
  }
...
function sanitizeRouteClass(url) {
  try {
    ...
    return pathOnly.replace(/[0-9a-f]{8}-[0-9a-f-]{27,}/gi, '<uuid>').slice(0, 120);
  } catch {
    return '<url>';
  }
}
...
function publicInterpretation(result) {
  ...
  if (result.parentCortexOnly === true || result.parentHasVisibleMainAnswer === false) {
    return (
  ...
  if (result.initialMainAnswerVisible === false || result.reloadMainAnswerVisible === false) {
    return (
...
async function verifyQaAgentProvisioning({ qaAuth, mainAgentId, requiredCortexAgentIdsByName }) {
  const requiredIds = [mainAgentId, ...Object.values(requiredCortexAgentIdsByName)];
  ...
  const liveCortexEntries = new Map(
    (Array.isArray(mainAgent?.background_cortices) ? mainAgent.background_cortices : [])
  ...
  const activationDriftNames = REQUIRED_CARD_NAMES.filter((name) => {
    ...
    return (
      String(activation.provider || '').trim() !== EXPECTED_ACTIVATION_PROVIDER ||
...
async function visibleCortexRowTexts(page) {
  return page.locator('.progress-text-wrapper button').evaluateAll((buttons) =>
    buttons
...
function evaluateVisibleState(text, cortexRowTexts = []) {
  ...
  const forbiddenMatches = FORBIDDEN_VISIBLE_PATTERNS.filter((pattern) => pattern.test(text)).map((pattern) =>
    pattern.toString(),
  );
  const mainErrorMatches = MAIN_ERROR_PATTERNS.filter((pattern) => pattern.test(text)).map((pattern) =>
    pattern.toString(),
  );
  return {
    cardNames,
    forbiddenMatches,
    mainErrorMatches,
    hasBackgroundAgentFooter: /Background agent:\s*(Red Team|Confirmation Bias)/i.test(text),
    hasTerminalState: /Analysis complete|Error occurred|Did not activate/i.test(text),
...
function uniqueRequiredNamesFromParts(parts, predicate) {
  ...
  for (const part of parts) {
    const name = normalizeCortexName(part?.cortex_name || part?.cortexName || part?.name);
...
async function verifyStoredCortexParts({ qaAuth, conversationId, prompt }) {
  ...
  const allMessages = await qaAuth.db
    ...
    .toArray();
  const messages = allMessages.filter((message) => message.isCreatedByUser === false);
  const userMessages = allMessages.filter((message) => message.isCreatedByUser === true);
  ...
  const parentMessage = originatingUserMessage
    ? messages.find((message) => message.parentMessageId === originatingUserMessage.messageId)
    : messages[0];
  ...
  const parentCortexParts = Array.isArray(parentMessage?.content)
    ? parentMessage.content.filter((part) => part && String(part.type || '').startsWith('cortex_'))
    : [];
  const parentHasVisibleMainAnswer =
    parentText.trim().length > 0 && !isNoVisibleAnswerMarker(parentText);
  const parentCortexOnly = Boolean(parentMessage) && parentCortexParts.length > 0 && !parentHasVisibleMainAnswer;
  ...
  const storedTerminalNames = uniqueRequiredNamesFromParts(
    ...
    (part) =>
      ...
      ['complete', 'error', 'skipped', 'did_not_activate'].includes(String(part.status || '')),
  );
  ...
  const storedErrorNames = uniqueRequiredNamesFromParts(
    cortexParts,
    (part) => part.type === 'cortex_insight' && String(part.status || '') === 'error',
  );
  const publicState = {
...
async function waitForStoredCortexParts({ qaAuth, conversationId, prompt, timeoutMs }) {
  const deadline = Date.now() + timeoutMs;
  let latest = null;
  while (Date.now() < deadline) {
    latest = await verifyStoredCortexParts({ qaAuth, conversationId, prompt });
    const hasTerminalCards = latest.storedTerminalNames.length === REQUIRED_CARD_NAMES.length;
    const hasSuccessfulCards = latest.storedCompleteInsightNames.length === REQUIRED_CARD_NAMES.length;
    if (
...
async function waitForConversationForPrompt({ qaAuth, prompt, startedAt, timeoutMs = 60000 }) {
  const deadline = Date.now() + timeoutMs;
  const startedDate = new Date(startedAt);
  while (Date.now() < deadline) {
    const userMessage = await qaAuth.db
      .collection('messages')
      .findOne(
        {
          user: qaAuth.userId,
  ...
  try {
    ...
    page.on('response', (response) => {
      if (response.status() >= 400 && isCriticalHttpRoute(response.url())) {
        httpErrorRoutes.push(`${sanitizeRouteClass(response.url())} ${response.status()}`);
      }
    });
    ...
    Object.assign(result, evaluateVisibleState(await visibleBodyText(page), await visibleCortexRowTexts(page)));
    for (const name of REQUIRED_CARD_NAMES) {
      const header = page
    ...
    await page.evaluate((requiredNames) => {
      for (const name of requiredNames) {
        const candidates = Array.from(document.querySelectorAll('.progress-text-wrapper button'))
          .filter((element) => (element.textContent || '').includes(name))
          .sort((a, b) => (a.textContent || '').length - (b.textContent || '').length);
        const button = candidates.find((element) => element.getAttribute('aria-expanded') !== 'true');
    ...
    await page.waitForFunction(
      (requiredNames) => {
        ...
        return (
          requiredNames.every((name) => body.includes(`Background agent: ${name}`)) &&
    ...
    Object.assign(result, state, {
      ...
      initialMainAnswerVisible: bodyContainsMainAnswer(text, parentMainAnswerText),
      consoleErrorCount: consoleErrors.length,
      failedRequestCount: failedRequests.length,
      failedRequestRoutes: summarizeRouteClasses(failedRequestRoutes),
      criticalHttpErrorCount: httpErrorRoutes.length,
      criticalHttpErrorRoutes: summarizeRouteClasses(httpErrorRoutes),
    });
    ...
    result.reloadMainAnswerVisible = bodyContainsMainAnswer(reloadBodyText, parentMainAnswerText);
    result.pass =
      ...
      storedState.storedCompleteInsightNames.length === REQUIRED_CARD_NAMES.length &&
