...
- Classifier-owned rows use direct `openAI` Sol/Terra execution and a reviewed
  `glasshive-harness / claude-code:opus / high` fallback. Deep Memory is the explicit exception: it
  uses `glasshive-harness / codex-cli:gpt-5.6-sol / medium` as its fallback.
