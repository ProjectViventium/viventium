...
| `SCHED-002` | Trigger and delivery ledger | Scheduler trigger, delivery ledger, visible notification/chat | test_scheduling_mcp_supervision.py plus isolated scheduled fixture | PASS-AUTOMATED/PARTIAL 2026-07-18; due-window/GlassHive/callback/parent-child ledger fixtures pass, isolated automatic visible delivery NOT RUN |
...
| `SCHED-004` | Runtime identity and port ownership | Launcher, `/health`, generated config, dev-env runtime | test_scheduling_mcp_supervision.py; test_stable_dev_runtime_workflows.py | PASS-AUTOMATED 2026-07-14; synthetic status requires exact semantic status, service identity, and configured-ledger hash before showing Running |
...
| `SCHED-009` | Callback outbox bounded termination and health gate | GlassHive callback outbox, scheduler delivery ledger, metrics | GlassHive callback regressions plus synthetic outbox-health fixture | PASS-AUTOMATED/PARTIAL 2026-07-18; retry/dead-letter/backlog fixtures pass, isolated due-window outbox probe NOT RUN |
| `SCHED-010` | Built-in nightly reflection is seeded for the installing local admin without personal identity drift. | Prompt Workbench first-admin seed, Scheduler task row, GlassHive executor metadata | `test_prompt_workbench.py`, `test_default_nightly_routines.py`, install/upgrade generated-env QA | PASS-AUTOMATED/PARTIAL 2026-07-18; generated defaults/seed fixtures pass, isolated browser/automatic schedule proof NOT RUN |
...
## `SCHED-010` - Custom Settings Scheduler Readiness Row

- Requirement: Custom Settings Install must expose Scheduler as a first-class installed surface,
  while immutable Easy Install states that Scheduler is not packaged rather than treating it as an
  implicit Workbench detail.
...
- Preconditions: generated runtime env and scheduler SQLite state are available with synthetic or
  sanitized rows.
- Steps:
  1. Compile an Easy Install (`install.experience: express`)-shaped config and confirm Scheduler is
     classified Custom Settings only; compile a selected Custom Settings config and confirm
     Scheduler env keys are generated.
  2. Run Custom Settings install/status summary and confirm the Scheduler row shows service state plus sanitized
...
- Last run: PASS-AUTOMATED/PARTIAL 2026-07-14; synthetic definition/bootstrap/provider-failure and
  child/parent/delivery/callback ledger regressions pass. Isolated automatic browser persistence is
  NOT RUN.
...
- Run one isolated scheduled synthetic control, then correlate the exact visible/stored response
  with sanitized logs proving the configured provider/model/effort tuple and no hidden fallback.
...
- Last run: PASS-AUTOMATED/PARTIAL 2026-07-14; 19 focused happy/unhappy tuple, fallback, and ordinary-
...
| `SCHED-UC-010` | Inspect Scheduler posture after Easy Install, then readiness after Custom Settings Install or upgrade. | `39_Installer_and_Config_Compiler.md` / `SCHED-010`, `INST-004` | `bin/viventium status`, scheduler health endpoint, scheduler SQLite ledger, Workbench run history | Sanitized endpoint result, DB count/status/outcome/next-run summary, generated env keys, focused tests. | Easy Install says Scheduler is not packaged; Custom Settings shows installed/configured/running/degraded with a concrete next action and no private schedule data in public output. | PARTIAL 2026-07-21; Custom Settings ledger/browser evidence exists and Easy Install registry truth passes, while the final exact-artifact Easy omission wording is not yet run |
