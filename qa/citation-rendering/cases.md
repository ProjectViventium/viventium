...
| `CITE-UC-003` | Ask a current-data prompt that triggers `web_search`, then verify citation UI appears only when sources were returned. | `docs/requirements_and_learnings/08_Citation_Rendering.md`, `docs/requirements_and_learnings/10_Open_Source_Web_Search.md` / `CITE-004` | LibreChat browser answer and citation expansion | Persisted `web_search` artifacts, returned-source counts, local search backend health, hosted search backend status, request logs, rendered citation chips/details | Source markers and expandable citations exist only for real returned web evidence; no source is fabricated when search fails. | FAIL (escaped 2026-05-18 by no-evidence search path; rerun pending) |
