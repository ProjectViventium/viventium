...
| `PERI-009` | Unattended analytical automations use the compiled Sol/xHigh route consistently. | Nightly quality is not silently downgraded by stale metadata, an absent route-proof flag, or ambient CLI config. | compiler, Workbench, memory hardening, Scheduling dispatch, GlassHive command/ledger, UI | Compiler/dispatch tests plus real process command and DB metadata | PASS 2026-08-09 ([nightly review](../memory-hardening/reports/2026-08-09-nightly-routines-health-review.md)); Workbench visibly preserved Sol/xhigh requested/effective effort and completed. Memory hardening independently used its configured Luna/medium route. |
| `PERI-010` | Wearable evidence enters through a consented, least-privilege connector or explicit private import and remains distinct from saved memory and health-context inference. Complete integration reads all six official families through every page and preserves source relationships and units. | Viventium can use complete, fresh, traceable device evidence without leaking raw biometrics, inventing health advice, or requiring a mobile app when a vendor cloud API suffices. | vendor API, private raw archive, read-only MCP, browser chat, Periphery health-context artifact | Component contract tests with multi-page fixtures and relationship/unit assertions, real owner-device acquisition/MCP A/B, then parent integration and revoke/correction QA | PARTIAL 2026-08-18; official owner consent, all six read families, rolling corrections, exact archive reads, bounded correlation, and a fresh live artifact were reverified after restart. Full multi-page history plus relationship/unit preservation and live revoke remain unproved; disconnect has synthetic regression coverage. |
| `PERI-011` | Degraded snapshot prerequisites downgrade artifact quality. | A fresh nightly artifact cannot appear healthy when its evidence corpus was unavailable. | Workbench periphery snapshot, artifact quality summary, browser detail | Focused artifact-quality regression plus live snapshot/artifact audit | PASS 2026-08-09 ([nightly review](../memory-hardening/reports/2026-08-09-nightly-routines-health-review.md)); current snapshot was complete with no missing prerequisites and the fresh artifact passed. The degraded-input branch remains regression-covered rather than live-exercised today. |
| `PERI-012` | Clean install, upgrade, and start materialize the pinned health component and executable runtime before advertising its MCP. | The health tools work on a clean machine instead of depending on an owner laptop's leftovers. | component bootstrap, lock pin, App Support runtime, generated MCP config | Bootstrap selection, isolated runtime install/status, MCP initialize/list/call, pin/artifact parity | PARTIAL 2026-08-10; the reviewed component head, parent pin, installed manifest/hash, public CLI, preserved archive, and read-only MCP/runtime tests agree. |
| `PERI-013` | Health-context snapshots are bounded, private, source-cited, failure-aware, and easy for Viventium to read across complete paginated history. They preserve relationships and units and support explicit correlation with other authorized Life context. | Correlation can use exact complete WHOOP evidence and authorized Life context without exposing credentials, paths, opaque record IDs, or unlimited payloads. | Workbench private snapshot, health CLI, snapshot manifest/API, authorized Life context | Synthetic archive fixtures for multi-page complete/empty/missing/partial/truncated/timeout states, relationship/unit and cross-Life correlation assertions, plus permission/privacy checks | PARTIAL 2026-08-10; the scheduled snapshot inspected 48 summaries, included 18 exact bodies, had zero read failures/truncations/missing prerequisites, and exposed metadata only in Workbench. Full paginated-history, relationship/unit, and authorized cross-Life correlation acceptance remains unrun. |
| `PERI-014` | WHOOP acquisition and health-context analysis have distinct scheduler owners and independent cognitive-health evidence. | Daily acquisition and correlation run once, recover honestly, never duplicate network pulls, and never hide a failed analysis behind successful acquisition or manual recovery. | Health LaunchAgent, Scheduling Cortex definition, Workbench run ledger, read-only cognitive-integrity report | Schedule rendering/status, seeded-definition reconciliation, catch-up, restart/persistence, no-pull assertion, owner-scoped scheduler provenance | FAIL 2026-08-25; the 06:00 owner acquisition succeeds, but six consecutive 06:15 analyses failed or lacked a verified callback. Automated joined-health regressions cover active failure, manual masking, scheduler provenance, success, and optional disabled state; installed recovery remains unproved. Historical [nightly review](../memory-hardening/reports/2026-08-11-nightly-routines-health-review.md). |
| `PERI-015` | The optional Life projection contains connector metadata only and updates from the private health-context path. | The user's Life health folder shows freshness and integration state without becoming a second raw biometric store. | private Life folder, App Support archive, Workbench snapshot | Synthetic projection shape/permissions/hash tests and real private status refresh | PASS 2026-08-10; the refreshed projection was `0700`/`0600`, reported complete with 18 bounded records included, and contained status/count/hash metadata with no raw body, record locator, or credential. |
| `PERI-016` | Health-context sidecars use an internally consistent expiry interval, and managed template repairs do not overwrite owner-customized prompts. | A completed worker cannot leave an invalid artifact hidden behind a successful run, and product upgrades remain safe for private customization. | Workbench prompt definition/version, GlassHive result, artifact validator, browser detail | Strict timestamp/duration prompt regression, managed-revision reconciliation, owner-edit preservation, real rerun | PASS 2026-08-10; the scheduler-owned rerun produced a fresh `P1D` artifact with 4/4 sources resolved, zero ungrounded claims, zero actions, and zero memory proposals; Workbench kept it passed after reload. |
| `PERI-017` | Native provider terminal errors keep a structured failure class through GlassHive, callback reconciliation, Scheduling Cortex, and Workbench without treating task prose as control evidence. | A blocked health-context run names the actionable provider condition instead of showing `unknown`, while ordinary prompt/output text cannot manufacture a provider outage. | GlassHive CLI evidence parser, worker callback, scheduled-run ledger, Workbench run history | Focused classification and callback regressions, exact sanitized log replay, real manual run plus reload | PASS 2026-08-10; the historical provider-limit attempt remains truthfully classified after reload, and later manual plus scheduler-owned runs completed successfully with accepted artifacts. |
...
| `PERI-019` | Local owners receive minimal-click, capability-complete WHOOP onboarding without embedded public secrets or hidden provider limits. | A provisioned owner uses one Connect action plus WHOOP consent; an unprovisioned owner gets a combined save/connect fallback, automatic history/schedule setup, readable six-family counts, and honest recovery state. | Settings UI, admin API, macOS URL helper, health CLI/runtime, LaunchAgent | Component/command/route/UI/helper tests, built artifacts, installed browser and persistence QA | PASS 2026-08-10; direct setup, owner consent handoff, readable six-family status, automatic schedule, refresh persistence, tests, and builds pass. |
| `PERI-020` | API, official export, and manual image evidence form one honest WHOOP coverage boundary. | Journal/export fields and app-only Stress Monitor context can reach Viventium without scraping, private endpoints, false normalization, or arbitrary file access. | Settings UI, admin API, exact archive, read-only MCP text/image | ZIP abuse regressions, image magic/size/hash regressions, MCP image parsing, browser uploads, private archive inspection | PARTIAL 2026-08-10; exact ZIP/image preservation, abuse limits, MCP parsing, and installed lanes pass, but a real export was not supplied and expired image attachments must be reattached. |
| `PERI-021` | Host-wide health tools are owner-only and recovery is bounded across every product entry point. | Sharing the main agent cannot share private health; disabled installs advertise nothing; expired/repeated setup recovers without races, infinite polling, shell launch, or duplicate exports. | MCP tool loader, compiler, admin API, Settings UI, helper/CLI/archive | Audience-policy role tests, enabled/disabled compile tests, all-mutation gates, OAuth TTL/single-flight/background lifetime, degraded/pending UI, exact-hash idempotency | PASS 2026-08-10; owner chat/tool startup passed, ordinary-account card/projection/process startup were denied, and refresh/recovery tests passed. |
| `PERI-022` | A Workbench Periphery run rejected by GlassHive's parallel-isolation policy recovers without weakening isolation or losing its private artifact. | Health Context and risk radar remain reliable when host execution is prohibited; the user sees a completed or honestly failed run, and private outputs return safely. | Prompt Workbench, Scheduling Cortex, GlassHive isolated worker, signed callback, private Periphery folder | Nested-error parsing, Docker retry/rebase, current-run pair validation, traversal/stale/missing-pair/socket regressions, real browser run plus reload | PASS 2026-08-18; the final post-restart Health Context run recovered from the structured host rejection, survived a pre-existing Chromium socket in the bind-mounted worker home, completed in 100.73 seconds, imported an exact owner-only pair, passed artifact quality, reconciled both ledgers, and persisted after reload. |
| `PERI-023` | `DATA-010`: one failed WHOOP prompt is traced from trigger through conversation, scheduler/connector logs, DB/state, and owning code, then rerun across happy, unhappy, recovery, persistence, and duplicate-suppression branches. | The user receives the real root cause and a durable correction; one green subsystem or manual success cannot hide the original failure. | Installed browser conversation, WHOOP connector, scheduler/Workbench, logs, DB/state, source, restart | One exact end-to-end fault and recovery ledger | NOT RUN — cataloged 2026-08-30 |
...
| `PERI-UC-003` | A nightly module encounters missing scheduler, GlassHive, memory, file, or snapshot prerequisites. | `53_Viventium_Periphery_Nightly_Insights.md` / `PERI-001`, `PERI-004` | Workbench scheduled run, Scheduler health, GlassHive host worker, snapshot harness | Failure class, health/status, retry/catch-up policy, sanitized logs, no-result artifact marker | The run is classified as blocked, skipped, or degraded with a concrete reason and no invented insight. | NOT RUN — cataloged 2026-08-09 ([nightly review](../memory-hardening/reports/2026-08-09-nightly-routines-health-review.md)); current prerequisites were complete and the run passed, so the degraded live branch was not triggered today. |
| `PERI-UC-004` | The user reviews whether a nightly insight should become memory, a task, a health-pressure signal, or stay as private scratch. | `53_Viventium_Periphery_Nightly_Insights.md` / `PERI-003`, `PERI-007` | Workbench artifact review plus memory proposal surface once implemented | Memory proposal dry-run, health module state, artifact labels, approval log | Durable memory and health-pressure state are updated only through governed proposals or approved module policy; raw scratchpad text is not silently promoted. | PARTIAL 2026-07-11: the risk-radar boundary passed (`memoryWriteMode=off`, zero proposal refs); health-pressure persistence was not run. |
| `PERI-UC-005` | The user or QA reruns the same insight eval against a private snapshot corpus that includes useful signal and junk/test conversations. | `53_Viventium_Periphery_Nightly_Insights.md` / `PERI-004`, `PERI-005` | Private snapshot harness and sanitized eval report | Corpus labels, junk/test exclusion counts, output rubric, false-positive/false-negative examples | The eval proves the module resists overfitting to QA noise and reports evidence quality, confidence, staleness, and opportunity/risk value. | PASS 2026-08-09 ([nightly review](../memory-hardening/reports/2026-08-09-nightly-routines-health-review.md)); deterministic evals passed 6/6 and the live nightly produced a fresh passed artifact. |
| `PERI-UC-006` | The user privately connects or imports a supported wearable with multi-page history, then asks Viventium to correlate that complete history with other authorized Life context for today's planning. | `53_Viventium_Periphery_Nightly_Insights.md` / `PERI-007`, `PERI-010`–`PERI-020` | Owner OAuth, Settings onboarding/export/image import, read-only MCP, health-context Workbench routine, browser chat, connector status, private evidence store, authorized Life context | Consent/scopes, pagination receipts, six-family counts, preserved relationships/units, source/freshness, tool trace, correlation inputs, missing/correction state, cognitive A/B score, revoke/delete evidence, sanitized logs | Every page and all six official families are readable with their relationships and units intact. The answer materially improves usefulness, cites measurement/source time, separates observations/scores/inferences, correlates only authorized Life context, stays non-diagnostic, and keeps disconnect and historical retention explicit. | PARTIAL 2026-08-10; live owner API, minimal-click UI, first-message chat, corrections, bounded correlation, Life projection, persistence, and cross-role isolation pass. Full pagination with relationship/unit proof, explicit cross-Life correlation, real export, and expired-image reattachment remain open. |
| `PERI-UC-007` | Send a WHOOP-backed planning prompt that fails, trace the exact failure through trigger, conversation, logs, DB/state, and code, repair it, then rerun happy, degraded, recovery, reload/restart, and duplicate controls. | `DATA-010` / `PERI-023` | Installed browser, connector, Scheduler/Workbench, logs, DB/state, source | Original visible failure, trigger and run identities, root-cause evidence, scoped change, recovery result, persistence, counts/hashes, and remaining gaps | The true root cause is named and corrected; every affected branch has direct evidence; reload/restart persists; no pull, run, artifact, or message is duplicated. | NOT RUN — cataloged 2026-08-30 |
...
## `PERI-022` - GlassHive Isolation Recovery And Artifact Return

- Escaped failure: Workbench retained a host-mode private workspace declaration after GlassHive
  began requiring isolated execution for untrusted parallel missions. The complete WHOOP snapshot
  reached dispatch, but worker admission ended at a generic-looking HTTP conflict and no fresh
  health artifact returned.
- Second escaped failure: after recovery was implemented, a restarted isolated worker retained a
  Chromium Unix socket beneath its bind-mounted home. Recursive `chown` on Docker Desktop failed
  with an invalid-argument substrate error before the worker could begin the prompt.
- Expected result: Scheduling Cortex preserves the structured
  `parallel_execution_isolation_required` class, retries the same run in Docker, rebases only the
  declared private output root, and imports the recognized current-run artifact pair after a valid
  signed completion callback. Worker identity is bound before assignment; early callbacks retry,
  required imports claim the ledger before side effects, and transient pair failures reconcile on a
  later signed retry.
- Forbidden result: relaxing GlassHive isolation, copying arbitrary worker files, selecting an old
  same-worker artifact, importing for a custom template based on prompt text, following an unsafe
  path, reporting success without both files, applying memory after import failure, or showing a
  generic conflict as the final user-visible reason. Sandbox preparation must not recursively
  change Unix-socket ownership or follow a planted link. A custom template or non-off memory mode
  must not inherit the built-in isolated artifact bridge.
- Evidence to capture: host rejection and isolated retry mode, structured template/module contract,
  exact current scheduled-run binding, full schema-v2 validation, paired-file count and owner-only
  modes, a socket-bearing bind-mounted worker-home probe, early/duplicate/retry callback behavior,
  callback/parent-child ledgers, Workbench completed/failed wording, artifact quality, and reload
  persistence.
- Automated regression owner: Scheduling Cortex `tests/test_dispatch.py`,
  `tests/test_workbench_artifacts.py`, and `tests/test_bootstrap.py`, plus GlassHive
  `runtime_phase1/tests/test_docker_sandbox.py`.
