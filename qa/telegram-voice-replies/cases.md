...
| `TGVOICE-UC-007` | With Smart voice for text enabled, request a copy-ready synthetic email, ordinary conversation, and an explicitly spoken reminder. | `TGVOICE-006` | Telegram Desktop and Prompt Workbench | Visible text/audio, clean DB row, voice-decision log, prompt lineage | Email stays complete/text-only; conversation and spoken reminder get at most one audio; controls remain hidden | PASS-HISTORICAL 2026-07-30; current candidate PARTIAL pending fresh Telegram Desktop QA ([report](reports/2026-07-30-smart-delivery-regression-restoration.md)) |
