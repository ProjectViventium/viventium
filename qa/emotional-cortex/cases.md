...
| `EMO-038` | Native and host presence | Core macOS V menu exposes direct Open Feelings for running and stopped states; plugin uses the exact V in supported Codex/browser surfaces and an explicit non-overwriting Claude status-line opt-in | Favicon described as OS tray, silent Claude settings overwrite, missing `/feelings` route, or stale source/prebuilt/installed helper |
| `EMO-039` | Reaction potency and exact migration | New state uses the owner-approved magnitude-neutral instruction; the exact weak legacy default upgrades; any other user instruction remains exact; authoritative and fallback appraiser prompts define proportional `slight`/`clear`/`strong` semantics with no minimum bias; every committed write logs public-safe strength and realized-delta counts | minimum-strength bias returns, fuzzy migration rewrites custom text, runtime post-amplifies model output, or category collapse becomes unobservable |
