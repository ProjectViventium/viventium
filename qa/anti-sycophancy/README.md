...
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------- | ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Config/schema/compiler  | Targeted release/config/governance/public-safety suites                                                                     | Every owning code/config change | `FAIL` 2026-08-29 — the current anti-sycophancy architecture contract is 8/12: Deep Memory result evidence, Reality Check web search, single Red Team identity, and Deep Research web search fail. Older broad-suite counts do not certify this source candidate.                                                                                                              |
...
| Full regression         | All `ANTI-*` cases plus affected background, recall, Red Team, Prompt Workbench, GlassHive, Telegram, and voice suites      | Before rollout                  | `FAIL` — current deterministic source contract fails before the remaining user-surface matrix can be accepted.                                                                                                                                                                                                  |
...
- Current result: `FAIL`. Release acceptance is not met. The 2026-08-29 source contract is 8 passed and
  4 failed. Deep Memory result evidence, Reality Check web search, single Red Team identity, and
  Deep Research web search do not match the required architecture. Historical natural Reality →
  Main, the repaired Reality → Main → Red fallback → Main chain, the adversarial loop control,
  Deep Memory positive/negative behavior, Phase B value-gating, and three ordinary no-consult
  controls remain scoped evidence for the older candidate.
