...
| German locale resilience | `PASS` / `PARTIAL` | German locale changed existing chat copy and Connected Accounts remained complete with English fallback; no raw `com_*` key appeared. This proves safe fallback, not completed translations for all new copy. |
