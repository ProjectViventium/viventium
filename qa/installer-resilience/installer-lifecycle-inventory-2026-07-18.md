...
### Current Isolated Candidate Reconciliation — 2026-08-08

The previously merged component inventory remains the base. The current candidate advances
LibreChat and GlassHive while retaining the previously accepted component refs below. The parent
and Native manifests declare `merged`, and each merge tree equals the reviewed PR head.
...
| --- | --- | --- | --- |
| LibreChat | `ccfd307c3b6bd5573c58dfc4f16b84fc691fef4f` | merged public-main; PR 97 | Adds shared OIDC principal binding, user-scoped account and inference brokerage, direct capability issuance, and principal-bound recurring workspace scheduling while retaining direct-conversation behavior. All hosted checks passed and the merge tree equals the reviewed head. Signed-artifact identity remains separate. |
...
| GlassHive | `91a8a7b8b74a0ca7c7239a5e080c2fc046bd27d1` | published review branch after PRs 50–74 plus the hosted completed-open and terminal-recovery regressions | Adds the user-scoped control plane, durable private workspaces, OAuth-protected MCP access, provider-account and capability brokerage, recurring work, bounded durable launch, restart recovery, passive migration rehearsal, fail-closed permanent workspace lifecycle, retry-safe atomic staging of sealed worker-image inputs, current-cookie CSRF authorization for authenticated watch actions, an attested worker-image package snapshot compatible with the digest-pinned base, private host-owned run markers that remain readable after rootless worker writes, fail-closed private worker-path ACLs in multi-user deployments without widening workspace or profile permissions, provider-hosted email access with administrator-preapproved immutable OIDC subjects, an optional default-off local browser factor attached only to those exact subjects with public signup/reset absent, an essential-first Connections UI with reviewed sign-in links, capability-driven worker routes, authenticated opaque launch handoffs, first-account defaulting, personal-only sole-account selection, responsive primary controls, an explicit phase-local shared short-reference store that preserves owner-scoped workspace/artifact links across the runtime and gateway tiers, descriptor-validated compatibility with exact root-owned hosted state parents/files, and lease-safe provider-account recovery that preserves credential homes across upgrades without requiring a fresh OAuth setup, including non-following cleanup of provider-created private cache symlinks, Docker-compatible repair-container bind-mount syntax, renewed exclusive verification with a post-provider-check reseal before Ready is published, descriptor-safe temporary ownership for the exact non-secret Codex runtime metadata required by the reviewed CLI, one exact active provider route shared safely by the mission and live desktop actions through readiness, cleanup, and grant revocation, a sandboxed rendered preview plus direct attachment download for bounded self-contained HTML deliverables, safe artifact-reference promotion instead of worker-local browser links, metadata-discovered Codex OAuth resource selection without duplicate authorization parameters, and completed workspace opening that uses the rendered card state as its authority, stays non-mutating, keeps actual paused/idle named workspaces deliberately resumable, and keeps Pause visible for terminal attention states whose recovery guidance requires it. Standalone MCP remains config-only, and configured role maps reject missing or unmapped MCP roles before principal enrollment. The affected runtime, UI, launch, lifecycle, Docker sandbox, CSRF, OAuth admission, provider-account recovery, mission, artifact, and rollout continuity suites passed. Built-artifact identity remains separate. |
...
This inventory closes review-head-to-merge-pin drift for PRs 97, 50, 9, and 10. It does not claim that
