...
| GlassHive | `17cd659fdfb4bf719d9c15f9ee8918c64cdcfb7a` | merged public-main; PRs 50–58 | Adds the user-scoped control plane, durable private workspaces, OAuth-protected MCP access, provider-account and capability brokerage, recurring work, bounded durable launch, restart recovery, passive migration rehearsal, fail-closed permanent workspace lifecycle, retry-safe atomic staging of sealed worker-image inputs, current-cookie CSRF authorization for authenticated watch actions, and an attested worker-image package snapshot compatible with the digest-pinned base while retaining origin-main compatibility. Standalone MCP remains config-only, and configured role maps reject missing or unmapped MCP roles before principal enrollment. The affected runtime, UI, launch, lifecycle, Docker sandbox, CSRF, and OAuth admission suites passed; each merge tree equals its reviewed head. Built-artifact identity remains separate. |
