...
| GlassHive | `35c82be4275f72ec3019e19580003d8947cd73d5` | merged public-main; PRs 50–62 | Adds the user-scoped control plane, durable private workspaces, OAuth-protected MCP access, provider-account and capability brokerage, recurring work, bounded durable launch, restart recovery, passive migration rehearsal, fail-closed permanent workspace lifecycle, retry-safe atomic staging of sealed worker-image inputs, current-cookie CSRF authorization for authenticated watch actions, an attested worker-image package snapshot compatible with the digest-pinned base, private host-owned run markers that remain readable after rootless worker writes, fail-closed private worker-path ACLs in multi-user deployments without widening workspace or profile permissions, provider-hosted email access with administrator-preapproved immutable OIDC subjects, and an optional default-off local browser factor attached only to those exact subjects with public signup/reset absent. Standalone MCP remains config-only, and configured role maps reject missing or unmapped MCP roles before principal enrollment. The affected runtime, UI, launch, lifecycle, Docker sandbox, CSRF, and OAuth admission suites passed; each merge tree equals its reviewed head. Built-artifact identity remains separate. |
