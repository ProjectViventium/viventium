...
| GlassHive | `1dc48ca90b7ec5445b18f46b37928c8c0802065c` | merged public-main; PRs 50–52 | Adds the user-scoped control plane, durable private workspaces, OAuth-protected MCP access, provider-account and capability brokerage, and recurring work while retaining origin-main compatibility. Standalone MCP remains config-only, and configured role maps now reject missing or unmapped MCP roles before principal enrollment. The complete runtime suite and focused OAuth admission tests passed; each merge tree equals its reviewed head. Built-artifact identity remains separate. |
