...
| GlassHive | `a746e3c1eb1ca663bedc94a639db7efd4da3197e` | merged public-main; PRs 50–55 | Adds the user-scoped control plane, durable private workspaces, OAuth-protected MCP access, provider-account and capability brokerage, recurring work, bounded durable launch, restart recovery, passive migration rehearsal, and fail-closed permanent workspace lifecycle while retaining origin-main compatibility. Standalone MCP remains config-only, and configured role maps reject missing or unmapped MCP roles before principal enrollment. The affected runtime, UI, launch, lifecycle, and OAuth admission suites passed; each merge tree equals its reviewed head. Built-artifact identity remains separate. |
