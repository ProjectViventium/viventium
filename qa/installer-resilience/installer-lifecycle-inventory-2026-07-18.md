...
The locally available parent history spans 2026-04-02 through 2026-07-19. At the
2026-07-19 18:39:51 -0400 snapshot, `git rev-list --count --all` returned 223 parent commits. The
path-filtered installer/delivery query below identifies all 118 locally available parent commits
that touched at least one audited installer surface. The extra parent-history entry since the prior
snapshot is a merge commit outside this path-matching ledger; it is not attributed to this audit:
...
| Component | Audit checkout versus configured pin | Viventium role | Fresh-install risk if current runtime is used as evidence |
| --- | --- | --- | --- |
...
- Safeguards, tenant isolation, workspace recovery, capability discovery, preflight/redaction, and UI
  hardening.
- Run links, status, evidence, worker-profile hardening, and July local work beyond the configured
  public pin.
...
## Current Documentation Contradiction Register
...
The current disposable-VM result is therefore a **source-candidate proof**, not delivery proof:
LibreChat must be committed intentionally, the parent pin reviewed, the client rebuilt, the signed
payload produced, and the installed artifact re-run before `INST-022` can pass.
