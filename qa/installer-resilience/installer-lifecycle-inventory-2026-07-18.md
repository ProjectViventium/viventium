...
| GlassHive | `f19656aa94f3802cf1980456e573097681e8d2b3` | reviewed branch tree; review worktree clean | Retains the user-scoped control plane, private workspaces, provider-account/capability brokerage, lifecycle/recovery/artifact foundations, and authorization hardening. It adds the native Codex/Claude package, direct one-call MCP guidance, exact launch follow-up context for clients without stable conversation headers, and the distinct Codex marketplace identity required for correct installed skill resolution. Focused runtime/UI compatibility suites, native fresh-process calls, plugin validators, and public-safety checks pass. Hosted artifact identity remains a separate gate. |
