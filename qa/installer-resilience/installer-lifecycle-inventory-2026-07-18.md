...
### Current Isolated Candidate Reconciliation — 2026-08-11

The previously merged component inventory remains the base. The current candidate advances
LibreChat, the modern voice playground, and GlassHive while retaining the previously accepted
component refs below. The parent and Native manifests declare `merged`; local clean-install proof
uses exact mirrors of the audited and hosted merged component trees.
...
| --- | --- | --- | --- |
| LibreChat | `541a4c4fdac97f54333d25a79de9c34e4319db04` | merged tree equals audited review head; review worktree clean | Retains the authoritative call task plane, durable cancellation/replay, browser capability and agent ACL boundaries, post-call memory hardening, structural speaker-authority gating, durable continuation fencing, server-authoritative gateway readiness, meeting-history seeding, rollback-safe startup, and the current public-safe package lock. It also carries the versioned model-owned Telegram delivery disposition, preserves ACL-checked structural handoffs for native providers while keeping ordinary specialist tools isolated, and recognizes the previously shipped managed-agent baseline during three-way upgrade reconciliation. Focused API/package suites, hosted checks, generator verification, secret scans, and independent review pass. Shipped-artifact identity remains a separate gate. |
...
| agent-starter-react | `95b324c7498c50a7421751e5dd71971e268a3a84` | merged tree equals audited review head; review worktree clean | Preserves the merged call-identity and cancellation foundation, then adds signed one-click call bootstrap, immutable server-owned routing, atomic Call/Wing/Listen-Only controls, structured task/source/speaker UI, reconnect paging, truthful recovery, bounded long-call rendering, accessibility, audio-first hangup, exact server-claimed worker preflight, and server-authoritative settled status. The token service now waits for the exact backend worker claim rather than treating LiveKit's sometimes-empty public job list as readiness. All 153 component tests, formatting, production build, hosted checks, public-safety checks, and first-run browser QA pass; installed-artifact identity remains a separate gate. |
| GlassHive | `11a40de9b55f6a7bb35bfe6b1ef7ab3cb03a9999` | reviewed branch tree; review worktree clean | Retains the user-scoped control plane, private workspaces, provider-account/capability brokerage, lifecycle/recovery/artifact foundations, and authorization hardening. It adds the native Codex/Claude package, direct one-call MCP guidance, exact launch follow-up context for clients without stable conversation headers, distinct Codex marketplace identity for correct installed skill resolution, and a versioned Claude package update. Native Claude setup now turns only its exact reviewed HTTPS authorization destination into the primary clickable sign-in action and removes terminal hyperlink controls from fallback output. Focused runtime/UI compatibility suites, native fresh-process calls, plugin validators, and public-safety checks pass. Hosted artifact identity remains a separate gate. |
| Viventium-Health | `91a9bbf5ff8bd0963dd3dc33bfd388c66fa7ed69` | merged tree equals audited review head; review worktree clean | Raw append-only WHOOP connector, owner-level daily scheduler, and read-only MCP are isolated and tested. The prerelease fix prevents a fresh alternate state root from inheriting an unrelated global LaunchAgent status. The 54-test run completed with 52 passes and 2 credentialed live-contract skips; clean wheel/sdist build, isolated install, and first-run status QA pass. Real OAuth/pulls remain private acceptance evidence. |
