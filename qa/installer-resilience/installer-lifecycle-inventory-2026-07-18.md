...
| --- | --- | --- | --- |
| LibreChat | `d4931a1c83d664e1fe052e9fd7c73cd9892d8eca` | merged tree equals audited review head; review worktree clean | Retains the authoritative call task plane, durable cancellation/replay, browser capability and agent ACL boundaries, post-call memory hardening, structural speaker-authority gating, durable continuation fencing, server-authoritative gateway readiness, meeting-history seeding, rollback-safe startup, and the current public-safe package lock, while aligning tracked local GlassHive provider capability policy with the compiled runtime. Clean install, package builds, API/client suites, focused memory/health/history suites, secret/history scans, and all hosted checks pass. Shipped-artifact identity remains a separate gate. |
...
| GlassHive | `3f4f74c90e6de15444bbd71fa12406d0c5d47337` | merged tree equals audited review head; review worktree clean | Unifies the user-scoped control plane, private workspaces, provider-account/capability brokerage, lifecycle/recovery/artifact foundations, signed exact-run actions, current workspace control-room UI and authorization hardening, isolated test state, and retry event-location telemetry. Runtime and UI suites, targeted telemetry tests, browser launch/completion/refresh/catalog/activity QA, persistence checks, secret/history scans, and hosted checks pass. Built-artifact identity remains a separate gate. |
