...
| GlassHive | `0442614dd6843d492c45b9c91c47fe6d1314d185` | reviewed feature branch; source tree clean | Retains the accepted Ultimate Phase 1 implementation and adds the reviewed workspace/run UX corrections: All workspaces by default, one Worker selector before Advanced, an always-visible effective account route, and lifecycle-only active status with diagnostics collapsed. The complete UI suite, real local browser path, syntax checks, parent-pin checks, and public-safety scans pass. Installed artifact and clean public install/restore remain separate gates. |
