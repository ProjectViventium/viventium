...
| GlassHive | `e9429b1f3d42559ee71bea63a54eb02d01b8744b` | merged public-main; PRs 50–67 | Adds the user-scoped control plane, durable private workspaces, OAuth-protected MCP access, provider-account and capability brokerage, recurring work, bounded durable launch, restart recovery, passive migration rehearsal, fail-closed permanent workspace lifecycle, retry-safe atomic staging of sealed worker-image inputs, current-cookie CSRF authorization for authenticated watch actions, an attested worker-image package snapshot compatible with the digest-pinned base, private host-owned run markers that remain readable after rootless worker writes, fail-closed private worker-path ACLs in multi-user deployments without widening workspace or profile permissions, provider-hosted email access with administrator-preapproved immutable OIDC subjects, an optional default-off local browser factor attached only to those exact subjects with public signup/reset absent, an essential-first Connections UI with reviewed sign-in links, capability-driven worker routes, authenticated opaque launch handoffs, first-account defaulting, personal-only sole-account selection, responsive primary controls, an explicit phase-local shared short-reference store that preserves owner-scoped workspace/artifact links across the runtime and gateway tiers, and descriptor-validated compatibility with exact root-owned hosted state parents/files. Standalone MCP remains config-only, and configured role maps reject missing or unmapped MCP roles before principal enrollment. The affected runtime, UI, launch, lifecycle, Docker sandbox, CSRF, and OAuth admission suites passed; each merge tree equals its reviewed head. Built-artifact identity remains separate. |
