...
| GlassHive | `69a8ddc94f208495c0aab9e3b19e1aa0e09dcd7c` | merged public-main; PRs 50–65 | Adds the user-scoped control plane, durable private workspaces, OAuth-protected MCP access, provider-account and capability brokerage, recurring work, bounded durable launch, restart recovery, passive migration rehearsal, fail-closed permanent workspace lifecycle, retry-safe atomic staging of sealed worker-image inputs, current-cookie CSRF authorization for authenticated watch actions, an attested worker-image package snapshot compatible with the digest-pinned base, private host-owned run markers that remain readable after rootless worker writes, fail-closed private worker-path ACLs in multi-user deployments without widening workspace or profile permissions, provider-hosted email access with administrator-preapproved immutable OIDC subjects, an optional default-off local browser factor attached only to those exact subjects with public signup/reset absent, and an essential-first Connections UI with reviewed sign-in links, capability-driven worker routes, authenticated opaque launch handoffs, first-account defaulting, personal-only sole-account selection, and responsive primary controls. Standalone MCP remains config-only, and configured role maps reject missing or unmapped MCP roles before principal enrollment. The affected runtime, UI, launch, lifecycle, Docker sandbox, CSRF, and OAuth admission suites passed; each merge tree equals its reviewed head. Built-artifact identity remains separate. |
