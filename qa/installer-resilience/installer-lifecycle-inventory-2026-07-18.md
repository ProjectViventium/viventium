...
| --- | --- | --- | --- |
| LibreChat | `37a49ccc738eca031022d9d1fa4a95f566dfc183` | merged tree exact; review worktree clean | PRs 75–80 added GlassHive as a core Provider/Model choice plus exact capability/effort, Opus 5, migration, scheduler, post-upgrade finalization, clustered-startup, canonical managed-agent provisioning, and capability-declared Chat Completions routing for main, cortex, handoff, and Phase B. PR 79 preserves the exact GlassHive wire model and effort while preventing SDK model-name heuristics from switching LibreChat to Responses. PR 80 binds first-turn substantive and Emotional Reaction cortices to the canonical conversation while preserving agent-scoped native sessions. All hosted build, lint, dependency, API, data-provider, and data-schema checks passed. Signed/notarized release and installed-runtime identity remain separate gates. |
