...
| --- | --- | --- | --- |
| LibreChat | `24289a2834aad5e620b6a4878bd20404a26b5106` | merged tree equals audited review head; review worktree clean | Retains the authoritative call task plane, durable cancellation/replay, browser capability and agent ACL boundaries, post-call memory hardening, structural speaker-authority gating, durable continuation fencing, server-authoritative gateway readiness, meeting-history seeding, rollback-safe startup, and the current public-safe package lock. It also carries the versioned model-owned Telegram delivery disposition through real request-body/header propagation, fallback and handoff routing, persistence, and replay. Package builds, focused and hosted API/client suites, secret scans, and all hosted checks pass. Shipped-artifact identity remains a separate gate. |
...
| GlassHive | `c16230fed7351a6b08606b6f3d99ae57246c7aa6` | merged tree equals audited review head; review worktree clean | Unifies the user-scoped control plane, private workspaces, provider-account/capability brokerage, lifecycle/recovery/artifact foundations, signed exact-run actions, current workspace control-room UI and authorization hardening, isolated test state, and retry event-location telemetry. It now emits the validated version-1 delivery disposition for audio-eligible messaging responses. The complete runtime suite, focused provider-contract tests, independent reviews, secret scans, and merge checks pass. Built-artifact identity remains a separate gate. |
