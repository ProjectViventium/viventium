...
| GlassHive | `425a4dc7340718244714c6b405805382abb91bf4` | reviewed feature branch; source tree clean | Retains the accepted Ultimate Phase 1 implementation and adds the reviewed workspace/run UX corrections: All workspaces by default, one Worker selector before Advanced, an always-visible effective account route, lifecycle-only active status with diagnostics collapsed, and a compositor-safe first paint. The complete UI suite, real local browser path, syntax checks, parent-pin checks, and public-safety scans pass. Installed artifact and clean public install/restore remain separate gates. |
