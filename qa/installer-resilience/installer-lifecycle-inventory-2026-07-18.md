...
component refs below. The parent and Native manifests declare `review-head-pending-merge`; local clean-install proof
uses exact local component mirrors until publication is separately authorized.
...
| --- | --- | --- | --- |
| LibreChat | `1cd439d03cc165f5f7bc4631d28dd678deb63827` | isolated clean review commit; publication pending | Adds the authoritative call task plane, durable cancellation/replay, browser capability and agent ACL boundaries, post-call memory hardening, structural unverified-speaker authority gating, per-participant shared-microphone tombstones, durable continuation fencing, server-authoritative gateway readiness, and build-independent upgrade migration helpers. The exact worker-claim status contract now matches the playground's pre-token gate; focused component tests, build, production-runtime, dependency, and public-safety checks pass. Public merge and shipped-artifact identity remain separate gates. |
...
| agent-starter-react | `56b728e358e8ff18cd5ac9c38655f26e492c277d` | isolated clean review commit; publication pending | Preserves the merged call-identity and cancellation foundation, then adds signed one-click call bootstrap, immutable server-owned routing, atomic Call/Wing/Listen-Only controls, structured task/source/speaker UI, reconnect paging, truthful recovery, bounded long-call rendering, accessibility, audio-first hangup, exact server-claimed worker preflight, and server-authoritative settled status. The token service now waits for the exact backend worker claim rather than treating LiveKit's sometimes-empty public job list as readiness. All 153 component tests, formatting, lint, and public-safety checks pass; public merge and installed-artifact identity remain separate gates. |
| GlassHive | `ce65fdf4b6567f513c10474069bf40495b48d960` | isolated clean review commit; publication pending | Preserves the merged user-scoped control plane, private workspaces, provider-account/capability brokerage, lifecycle, recovery, and artifact foundations, then adds signed exact-run cancel/retry actions, idempotency, completion-race CAS, callback correlation, graph-control projection, canonical capability decoding, and cleanup-vs-retry fencing required by Calls. All 1,210 component tests and dependency/secret scans passed; public merge and built-artifact identity remain separate gates. |
| Viventium-Health | `8b1f669d539a6a9e2e80e92c354130e4fac1a8af` | isolated clean review commit; publication pending | Raw append-only WHOOP connector, owner-level daily scheduler, and read-only MCP are isolated and tested. The prerelease fix prevents a fresh alternate state root from inheriting an unrelated global LaunchAgent status. All 53 component tests and clean wheel/sdist install QA passed; real OAuth/pulls remain private acceptance evidence and public merge remains a separate gate. |
