...
| GlassHive | `987c98b399c672cc45344b69c5dcb5e9612bdf9c` | merged tree equals audited review head; review worktree clean | Unifies the user-scoped control plane, private workspaces, provider-account/capability brokerage, lifecycle/recovery/artifact foundations, signed exact-run actions, current workspace control-room UI and authorization hardening, isolated test state, and retry event-location telemetry. It emits the validated version-1 delivery disposition and now deterministically owns operational SQLite connections, keeps one application-instance Store lifetime WAL owner, joins detached reconciliation before shutdown, and proves all service loops quiescent before closing the Store. The complete runtime suite, deterministic concurrency regressions, independent reviews, and secret scans pass. Built-artifact identity remains a separate gate. |
