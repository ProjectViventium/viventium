...
| --- | --- | --- | --- |
| LibreChat | `71501b3cabf0e309f23c27231a49cd6d0ad31b78` | public `main` tip; reviewed tree clean | Retains the authoritative call task plane, durable cancellation/replay, browser capability and agent ACL boundaries, post-call memory hardening, structural speaker-authority gating, durable continuation fencing, server-authoritative gateway readiness, meeting-history seeding, rollback-safe startup, and the current public-safe package lock. It also carries the versioned model-owned Telegram delivery disposition, preserves ACL-checked structural handoffs for native providers while keeping ordinary specialist tools isolated, recognizes the previously shipped managed-agent baseline during three-way upgrade reconciliation, and mirrors GlassHive's concise one-tool/one-call MCP guidance. Focused prompt/release checks, secret scans, and independent review pass. Shipped-artifact identity remains a separate gate. |
