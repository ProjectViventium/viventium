...
| `INST-014` | Easy Install Native and the planned Easy Install Docker profile share the safe onboarding/readiness/rollback contract, while Docker prerequisites never block Native first chat. | A novice installs Easy Native without Docker or developer tools, connects OpenAI, receives a persistent answer, and can deliberately choose the separate Easy Docker profile when that artifact is shipped. | schema/wizard, preflight/compiler, native services, manifest/bootstrap, browser setup/chat, status, restart/restore | Profile contract tests, exact-payload tests, and disposable Tart browser/install matrix | PARTIAL 2026-07-22; the source-candidate Docker delta and raw provider-card answers pass, a provisional-payload backup restored into an independent target, all 11 clean nested review heads are hosted in open PRs, and corrected LibreChat CI/review-head pin alignment passes. The current immutable payload is Native-only; independent merges and post-merge pins, signed Native payload, optimized OpenAI answer, shipped Easy Docker artifact, full fault matrix, and physical Docker Desktop remain open. |
...
| `INST-022` delivery alignment | Nested commit, pin, build, payload and installed runtime hashes agree. | A dirty source candidate is treated as the public artifact. | Commit/hash ledger at all five surfaces. | PARTIAL; 11 clean hosted review heads and current parent refs align, including corrected LibreChat `44ac1f7a...`, whose 15 hosted checks pass; approvals, merges, post-merge pins, and replacement build/payload/shipped/installed identity remain open. |
...
- Last run: PARTIAL 2026-07-22. The dated lifecycle reports use synthetic values and public-safe
  ...
  reconstructed source passed `python3 -m pytest tests/release/ -q` with 1,539 passed, 11 skipped,
  and 0 failed in 275.92 seconds. Staged and remote parent-PR exactness remain open.
...
- Last run: PARTIAL 2026-07-20. Profile-aware wizard/compiler/preflight/native startup and status,
  ...
  That rejected-payload finding is historical. Current lock refs equal all 11 nested `HEAD` values,
  but LibreChat and both playgrounds contain uncommitted reviewed changes; origin/merge and
  replacement artifact identity remain unrun.
...
- Last run: PARTIAL 2026-07-22. The prepared two-origin runtime, focused browser paths, Native proxy
  integrity tests, complete 1,539-test parent suite, 311-test corrected pin/payload slice, and exact
  modern-playground headed run pass. Corrected LibreChat `44ac1f7a...` is locally and hosted green
  and pinned; the exact
