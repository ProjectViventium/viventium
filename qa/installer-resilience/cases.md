...
| `INST-017` | Provider onboarding completes the full lifecycle and proves a useful persistent answer. | Connect, cancel/deny, test, wrong account, expire/revoke, quota/rate/network failure, retry, reauth, repair, disconnect, upstream revoke, local-secret deletion, restart, and first/second persistent chat are understandable and recoverable without changing unrelated state. | Connected Accounts, provider OAuth, secret store, chat, persistence, status | Real synthetic provider lifecycle browser QA | PARTIAL 2026-07-19; registration, setup handoff, disconnected guidance, OpenAI start/cancel recovery, Feelings, and persistence pass; grant, live test, wrong-account, first/second answer, expiry/revoke, reauth/repair, disconnect/secret deletion, and restart remain open |
...
| `INST-023` | Conditional onboarding and recovery stay sparse, premium, dismissible, jargon-free, keyboard/screen-reader accessible, responsive, localized, reduced-motion safe, and high-contrast usable while preserving a drafted prompt. | Nontechnical users see only the setup they need, can dismiss/resume it, and return to the same draft without a second setup surface. | installer/helper/browser dialogs, branded `setup=accounts`, Connected Accounts, and chat draft | UX/accessibility/localization browser and native UI matrix | PARTIAL 2026-07-20; browser scope passes ten-session/fresh-registration handoff stress, keyboard focus/activation, desktop/320/390 settled Axe with zero serious/critical findings, forced colors, zero horizontal overflow, German fallback, reduced motion, provider save/delete fault recovery, Feelings degraded recovery, refresh, and runtime-restart credential persistence. Conditional/dismissible draft-return, VoiceOver/native helper/Keychain/TCC, and Intel remain unrun ([report](reports/2026-07-20-inclusive-easy-install-browser-qa.md)) |
...
| `INST-025` | First launch through a persisted Ready-to-chat answer meets the declared setup-time budget on each supported Apple Silicon/macOS lane. | A new user gets a useful answer within the measured activation promise without hidden downloads, startup work or excluded failures. | exact signed app, provider connection, first answer, persistence, process/download trace | At least 20 independent cold clean-state runs per lane, nearest-rank p95, raw samples and fail-closed timeouts | NOT RUN — cataloged 2026-08-23; stable case added from the activation release contract |
| `INST-026` | Activation implementation starts from a pinned Hermes donor audit and reuses bounded MIT code/contracts/tests when this is safer and shorter, without importing the Hermes product or runtime. | Viventium catches up faster while keeping one V app, V state, LibreChat, GlassHive, Workbench and exact release authority. | donor ledger, notices/SBOM, installer, onboarding, updater, backup/restore, uninstall, service lifecycle, diagnostics, exact V app | Provenance/dependency scan, copied-contract tests, V integration tests and exact-app user QA | PARTIAL 2026-08-23; donor commit, license boundary and reuse candidates are documented; file-level ledger, copied implementation and exact-app proof remain open |
| `INST-027` | LIFE setup uses one validated CLI/API/helper boundary and an optional intent-only `WHAT_TO_CONNECT.md` managed block without scanning or claiming connection. | A user can understand, open, choose, defer, and record what they may connect later without exposing paths or silently reading files. | `viventium life`, authenticated setup/LIFE API, Mac helper, native folder picker, LIFE directory | State/owner/path contract tests plus headed helper/browser, restart, upgrade, and no-read/no-ingestion evidence | NOT RUN — cataloged 2026-08-30; current bootstrap covers only part of the accepted setup contract |
| `INST-028` | A supported user Stop persists across helper, login-item, watchdog, and component-supervisor activity until an explicit Start/Restart. | Stopped Viventium services and managed dependencies stay stopped; the product does not fight the user's lifecycle choice. | Viventium app/helper, public CLI, login item, watchdogs, component supervisors, process table | Lifecycle-state contract tests plus headed helper/CLI stop, elapsed watchdog window, login/restart, and explicit recovery QA | NOT RUN — cataloged 2026-08-30 |
| `INST-029` | `GOV-014`: when delivery requires the latest local Viventium running, start it through the supported path and prove active process, config, build, pins, and visible version identify that exact candidate. | The user receives the requested running candidate, not edited source beside an older runtime. | Public lifecycle CLI/helper, process table, config/build manifests, installed browser | Exact candidate activation and identity ledger | NOT RUN — cataloged 2026-08-30 |
...
`INST-015`–`INST-028` are separate authoritative rows in the Case Catalog above, so one broad
`PARTIAL` cannot hide a missing release proof. Do not maintain a second status table here. The
detailed case sections below add steps and evidence without replacing the dated catalog result.
...
## `INST-025` - Reproducible Setup Performance

- Requirement: from first app launch through one persisted real answer, setup meets the declared
  budget on every supported Apple Silicon hardware/macOS lane.
- Risk covered: a fast claim hides cold downloads, service startup, failures or slow provider work.
- Steps: run at least 20 independent cold clean-state attempts per lane; include all required bytes,
  startup and failures; subtract only recorded provider human-input wait; compute nearest-rank p95.
- Expected result: the raw ledger and visible journey agree and every lane meets the fixed budget.
- Forbidden result: warm, pooled, cherry-picked or build-only runs are called public activation p95.
- Evidence: exact artifact digest, environment, raw timestamps, bytes, failures, answer persistence
  and p95 calculation.
- Last run: NOT RUN — cataloged 2026-08-23.

## `INST-026` - Bounded Hermes Activation Reuse

- Requirement: before custom activation work, inspect the pinned Hermes donor and classify each
  candidate file or contract as `copy`, `adapt`, or `reject` with provenance and measured work saved.
- Risk covered: V reinvents mature lifecycle work, or a shortcut silently imports another product,
  unsafe dependency, incompatible state model or missing license notice.
- Steps: pin the donor commit; review file license, dependencies, assets and security assumptions;
  port donor regression tests; integrate only through V-owned lifecycle/state interfaces; run the
  exact V app path; record attribution, SBOM and rejection reasons.
- Expected result: bounded reuse reduces work while the user installs and operates only Viventium.
- Forbidden result: floating Hermes source, Hermes branding/identity/state, agent-host dependency,
  copied credentials, unattributed substantial code, or Hermes tests replacing V user QA.
- Evidence: donor ledger, source hashes/diffs, notices/SBOM, dependency and security review, old/new
  effort estimate, contract tests, exact app digest and clean-machine user evidence.
- Last run: PARTIAL 2026-08-23. Official donor commit and MIT boundary were verified and candidate
  capability families were recorded. No file-level decision ledger or implementation is complete.

## `INST-027` - LIFE Activation And Intent-Only Setup

- Requirement: one owner-scoped `viventium life`/API/helper contract controls LIFE state, safe legacy
  migration, helper actions, explicit folder choice, and the optional intent-only managed block.
- Risk covered: Viventium scans or ingests files without consent, leaks a private path, overwrites
  manual notes, stores wishes in an instruction/source file, or tells a remote user local folders
  were selected when no Mac picker ran.
- Preconditions: disposable new/existing LIFE roots, a supported legacy root, synthetic manual text,
  denied/symlink/full-disk fixtures, local helper/browser, and a remote-phone view.
- Steps:
  1. Exercise status, enable/defer, validated root selection, legacy migration, open, and explanation
     through CLI/API/helper and compare the single persisted state.
  2. Submit categories, a short note, Select/Clear all, and explicit folders; verify only one managed
     block changes and manual text remains byte-identical.
  3. Prove no scan/read/upload/index/ingestion/watch occurs and no selected item is labelled connected.
  4. Exercise cancel, invalid path, permission denial, symlink, full disk, restart, upgrade, and repeat.
  5. Verify remote-phone copy sends native folder choice back to the Mac and exposes no local path.
- Expected result: state and wording are truthful, owner-only, idempotent, and recoverable; existing
  LIFE content survives every path.
- Forbidden result: implicit filesystem discovery, ingestion, fake connected status, whole-file
  replacement, wishes in `AGENTS.md`/`99_System/sources.yaml`, path leakage, or remote fake success.
- Evidence: before/after file hashes, read/ingestion audit, owner/path receipts, helper/browser state,
  restart/upgrade result, and public-safety scan.
- Last run: NOT RUN — cataloged 2026-08-30.

## `INST-028` - Persistent User Stop

- Requirement: a Stop issued through the Viventium app/helper or public CLI writes one shared
  intentionally-stopped state before termination. Every auto-start owner respects it until an
  explicit Start/Restart clears it.
- Risk covered: the helper, login item, Docker integration, watchdog, or component supervisor
  silently relaunches work that the user deliberately stopped, or the UI promises that an
  indistinguishable raw process kill is a persistent product stop.
- Preconditions: a disposable installed candidate with every configured auto-start owner enabled,
  synthetic state, and process/log visibility.
- Steps:
  1. Stop from each supported user surface and verify the shared state before processes terminate.
  2. Wait through multiple watchdog intervals; reopen the helper; exercise login and runtime restart.
  3. Verify no stopped component or managed dependency relaunches and status says intentionally stopped.
  4. Use explicit Start/Restart; verify the state clears once and normal recovery resumes.
  5. Exercise a true crash separately and verify crash recovery does not erase the user-stop contract.
- Expected result: user intent, persisted lifecycle state, process reality, status, and logs agree.
- Forbidden result: polling or login activity resurrects stopped work; Stop is only an in-memory flag;
  an Activity Monitor kill is promised as persistent when the runtime cannot prove user intent.
- Evidence: state transition receipt, process identities, elapsed no-relaunch window, helper/CLI status,
  login/restart result, explicit recovery, and public-safe logs.
- Last run: NOT RUN — cataloged 2026-08-30.

## `INST-029` - Exact Local Candidate Is Running

- Requirement: `GOV-014` in `38_Public_Productization_and_Release.md`.
- Steps: record source/pins/build/config identities, activate through the supported lifecycle command,
  inspect active processes and browser-visible version, restart once, and compare every identity.
- Expected result: all layers identify the exact requested candidate and it is visibly running.
- Forbidden result: source-only delivery, stale process/build/config, or an unsupported manual launch.
- Last run: NOT RUN — cataloged 2026-08-30.

## `INST-030` - Complete Product After First Answer

- Requirement: `ONB-006` in `38_Public_Productization_and_Release.md` and
  `39_Installer_and_Config_Compiler.md`.
- Steps: complete first answer, then exercise Main, continuity, memory, calls, Wing, Listen-Only,
  Telegram, workers, and local-phone/remote use, including lazy start, one degraded dependency,
  recovery, reload, and restart.
- Expected result: each path remains first-class, truthful, persistent, and independently recoverable.
- Forbidden result: packaged replacement, silent capability narrowing, fake Ready, or one surface
  used as aggregate proof for another.
- Last run: NOT RUN — cataloged 2026-08-30.

## `INST-031` - Exact Release Acceptance Matrix

- Requirement: `ONB-009` in the public productization owner.
- Steps: run every named new/existing-user, desktop/mobile, keyboard/screen-reader/reduced-motion,
  provider, Life, Telegram media/voice, Call/Wing/Listen-Only, memory/recall/scheduling/Workbench/
  GlassHive, restart, upgrade, backup/restore/rollback, LAN/off-LAN, exact-artifact, and supported-
  platform branch under isolated synthetic state.
- Expected result: every branch has its own candidate-bound result, evidence, cleanup, and gap.
- Forbidden result: personal-state contamination, public private-data leakage, aggregate PASS, or an
  unrun branch omitted from the matrix.
- Last run: NOT RUN — cataloged 2026-08-30.

## `INST-032` - Surgical Clean-Base Reconciliation

- Requirement: `ONB-010` in the public productization owner.
- Steps: inventory and recoverably back up every relevant branch/worktree/commit/private-state
  boundary; classify each delta; create fresh current component and parent bases; replay only
  accepted changes; then prove component commit, parent pin, build, install, and fresh QA agreement.
- Expected result: all valuable state is recoverable and the installed candidate contains only
  reviewed accepted deltas with exact provenance.
- Forbidden result: reset, blanket/wholesale merge, selective loss, dirty-state shipping, or a pin
  that does not identify the tested component.
- Last run: NOT RUN — cataloged 2026-08-30.
...
| `INST-UC-017` | On a clean supported Mac, open the exact app, connect one approved account and receive a persisted answer while timing the full journey. | `38_Public_Productization_and_Release.md` / `INST-025` | Signed app, provider browser authorization, branded chat, restart | Artifact digest, raw timestamps/bytes/failures, process trace, provider state, answer persistence and p95 ledger | Every supported lane meets the declared activation budget without omitted cold work or failures. | NOT RUN — cataloged 2026-08-23 |
| `INST-UC-018` | Review one activation feature from pinned Hermes source through donor decision, V integration, install, failure, recovery and restart. | `38_Public_Productization_and_Release.md` / `INST-026` | Donor ledger, notices/SBOM, exact V app and affected lifecycle surface | Source provenance, dependency/security delta, copied tests, V integration evidence, visible behavior and persistence | Reuse saves work and remains invisible as an implementation detail; V owns identity, state, health and recovery. | PARTIAL 2026-08-23; source/license and capability-family review only |
| `INST-UC-019` | Open LIFE setup, defer once, return, choose explicit Mac folders, record categories/note, restart, and inspect the result from local and remote views. | `39_Installer_and_Config_Compiler.md` / `INST-027` | `viventium life`, authenticated setup/LIFE API, Mac helper/browser, native picker, remote-phone view | Canonical state, managed-block diff, filesystem read/ingestion audit, owner/path logs, restart/upgrade, public-safety scan | Only explicit intent is recorded; manual text and existing LIFE survive; nothing is scanned or called connected; remote copy sends folder choice to the Mac. | NOT RUN — cataloged 2026-08-30 |
| `INST-UC-020` | Stop Viventium from each supported user control, wait through watchdog/login activity, then explicitly start it again. | `39_Installer_and_Config_Compiler.md` / `INST-028` | Viventium app/helper, public CLI, process table, login item, watchdogs | Persisted intentionally-stopped state, process identities, logs, elapsed no-relaunch window, restart receipt | Stopped work stays stopped across every automatic owner; only explicit Start/Restart resumes it, while true crash recovery remains separate and truthful. | NOT RUN — cataloged 2026-08-30 |
| `INST-UC-021` | Ask for the latest local candidate to be running, then open it and compare source, pin, config, build, process, and visible version before and after restart. | `GOV-014` / `INST-029` | Supported lifecycle CLI/helper and installed browser | source/pin/build/config hashes, process command/path, runtime version, restart, and visible result | The exact requested candidate is active and visible; source changes alone never count as delivery. | NOT RUN — cataloged 2026-08-30 |
