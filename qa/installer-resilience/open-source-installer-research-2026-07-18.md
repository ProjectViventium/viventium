...
can improve Viventium's macOS Easy Install and onboarding. Three repositories were shallow
cloned to disposable `<temp>` storage for static inspection only; nothing was run or copied into
Viventium. GitHub stars are a directional adoption signal, not proof of security, usability, or
fitness. License constraints are treated as design boundaries.
...
Disposable static inspection used AnythingLLM `28fbff4`, Jan `84a98bf`, and Open WebUI `ecd48e2`.
These identifiers make the research reproducible; no source was copied into Viventium.
