...
| GH-STD-013 | Same | Durable authenticated View / Steer short refs can reopen retained workspaces while raw signed tokens and active sessions remain bounded. | Short refs, signed tokens, watch UI, websocket/session stream | Automated/API/browser timing tests | UPDATED 2026-06-20; local regressions cover default non-expiring `/r/{ref}`, auth gate, wrong-user rejection, fresh session-cookie minting after token expiry, configured short-ref TTL, legacy-row migration, and runtime-created/UI-resolved shared-state behavior. Full browser/live timing rerun pending. |
| GH-STD-014 | Same | Users can save default worker and effort preferences without affecting other users, and MCP plus Glass Drive UI apply the same per-profile effort policy. | Direct UI, MCP, API, DB/state | Automated/API/MCP/browser tests | PASS/PARTIAL 2026-05-24; automated API/MCP/UI tests and live MCP preference smoke passed, multi-browser two-real-user UI proof remains client-specific. UPDATED 2026-06-19: local regression scope includes Codex `none` parity and Claude `max` projection to runtime env from both MCP and Glass Drive UI, and the Glass Drive UI selector exposes Codex `none`. |
...
| GH-STD-017 | Same | Deployment default worker profile is honored when the caller omits `profile`, and legacy `backend=openclaw` metadata never overrides the selected worker profile/runtime. | UI, MCP, API, worker runtime, provider route, logs | Automated default-path API/MCP/UI tests plus live worker smoke | PASS/PARTIAL 2026-05-25 UTC on a live enterprise deployment; default-path MCP launch omitted `profile`, resolved to `codex-cli`, completed through the validated Responses route, and cleanup terminated the QA worker. UPDATED 2026-06-19: local regression scope now includes direct API omitted-profile creation, legacy blank project defaults in the built-in UI, alias reprofile runtime refresh, and live log routing from current profile instead of stale backend/runtime metadata. |
| GH-STD-018 | Same | Failed long-running worker tasks report a structured failure class, completion waits do not mislabel still-running work as failure, fresh user-facing artifacts are delivered even if the provider disconnects or the worker I/O capture closes after creation, and retryable work can be explicitly continued in the same workspace without overfitting the prompt or recursively bloating continuation instructions. | Direct MCP, LibreChat MCP, worker runtime, DB/state | Automated failure-class/wait/continue tests plus user-grade browser QA | PASS/PARTIAL 2026-05-26; automated runtime/API/MCP regression passes locally and on a live enterprise deployment for Codex runtime-label drift, provider failure classification, timeout config, failed-run artifact links, continuation effort override, continuation de-duplication, fresh artifact promotion, nested HTML path preservation, stale artifact rejection, and retryable runtime I/O failure classification; authenticated browser QA remains required when a signed-in browser profile is available. |
| GH-STD-019 | Same | Host-default deployments still honor explicit sandbox/Codex Workspace requests and preflight or safely recover missing/incompatible host runtime substrate before creating dead work or asking the user to fix global state. | Direct MCP, LibreChat MCP, API, DB/state | Automated MCP/API tests plus user-grade browser QA | UPDATED 2026-05-27; missing-CLI preflight exists, version-mismatch/managed-recovery browser QA pending. |
...
| GH-STD-024 | Same | Master first-delivery wildcard deep-work document QA proves GlassHive can accept wildcard input, perform spectacular complete deep work, and deliver wildcard text/files/document formats without overfitting to one prompt or file type. | LibreChat MCP, direct MCP, direct UI, host/workstation workers, generated artifacts, logs/DB | Manual/browser/computer QA, Claude/Codex CLI comparison, artifact validation, native skill/browser-extension readiness, public-safe research notes | PASS/PARTIAL 2026-06-15; direct MCP/API and workspace document runs passed, fake-document promotion guard and permanent 403 dead-lettering exist, and this pass added command-boundary completion contract coverage, assign/schedule bootstrap+effort preservation, host CLI stable/capability preflight floors, and Telegram callback dedupe regressions. 2026-06-15 native-capability follow-up added managed Claude/Codex browser-extension policy, worker skill inventory tests, Codex native-surface preservation by default with explicit lockdown-only `WPR_CODEX_CLI_DISABLE_FEATURES`, and Claude Code `max`/`--effort` fail-closed coverage; live LibreChat/direct-UI extension bridge QA remains. ClaudeViv flagged stale GlassHive test doubles; they were fixed and the full profile/MCP target files passed. 2026-06-15 private enterprise evidence, stored outside the public repo, proved LibreChat GlassHive row redaction/labels, provider-effort fallback logging, a GPT-5.4 Responses route with Codex Docker high-by-default plus explicit xhigh worker smokes, Claude Code workspace native `--chrome` plus `--effort max`, and a post-capacity wildcard first-delivery report run that produced validated DOCX, PDF, HTML, and Markdown artifacts with browser/PDF/DOCX inspection. 2026-06-16 enterprise Chrome smoke proved the workspace-worker path can create and preview a file after MCP schema/tool-description repair, and follow-up DB/runtime evidence found host prompt guidance was still overriding the deployment default with `effort=medium` for an ordinary bounded task. The root fix is to omit per-run effort for ordinary bounded work and let user/deployment defaults own baseline effort, while still explicitly selecting high/xhigh/max for deep-work classes. 2026-06-16 post-fix authenticated enterprise LibreChat/Chrome rerun passed for a fresh ordinary file request after owner-scoped quota recovery: visible chat showed one `Ran GlassHive workspace` row, one `Ran GlassHive wait` row, clean file and View / Steer link labels, no raw tool-call code, artifact preview showed exact marker content, View / Steer resolved to a completed `codex-cli` workspace, and DB evidence showed `gpt-5.4`, Docker execution, and `BOOTSTRAP_CODEX_EFFORT=high`. 2026-06-16 renderer follow-up added a live-announcement sanitizer at the mounted `LiveAnnouncer` chokepoint, kept `LiveMessage`/`MessageBlock` as defense-in-depth hardening, passed a targeted a11y regression, built the frontend, deployed a compatible enterprise LibreChat overlay, and verified the signed-in page loaded the new bundle with no raw signed-link or token text in hidden live/offscreen DOM. Remaining gaps: authenticated LibreChat conversational rerun for the full master deep-work document prompt, direct UI rerun, live Telegram dedupe rerun, brokered connected-account happy-path rerun after OAuth recovery, PPTX/file-input variants, and full export/recall sanitization. |
| GH-STD-025 | Same | GlassHive public MCP/API/UI payloads expose short link refs and compact status/launch/wait results without leaking raw signed tokens, stale backend labels, or internal IDs unless diagnostics are explicitly requested. | MCP, API, callbacks, preview pages, UI/WebSocket logs, worker profile metadata | Automated API/MCP/UI tests plus browser/log QA | PASS/PARTIAL 2026-06-20: local automated regressions cover `/r` and `/v1/link-refs` indirection, callback/live/preview short refs, MCP launch/wait/status minimization, signed-token/cookie log redaction, Codex backend/profile truth, and capacity retry race hardening. Approved-enterprise Chrome/log/DB QA passed for happy path, auth failure, cancellation/interrupt, steer/continue, direct `X-GlassHive-*` headers, default profile fallback, invalid profile fail-closed, and a true 305.1-second long-wait timeout. Live quota saturation was not forced; quota/concurrency failure remains covered by local regression and should be forced only in a maintenance/disposable-capacity window. Release durability remains PARTIAL until the nested GlassHive source state is committed/reconciled and the live VM is pinned to a reproducible revision. |
...
| GH-STD-UC-023 | Give GlassHive a wildcard input prompt or file(s), ask for deep research/work, and require a wildcard output such as text, PDF, Word, PowerPoint, or multiple files. | GH-STD-024 | LibreChat MCP, direct MCP, direct UI, host/workstation workers, generated artifacts, browser/computer validation | User prompt/input files, worker logs, generated files, opened document validation, final report, DB/run state, Claude/Codex comparison, public-safe research notes. | Worker delivers a spectacular, complete, deeply researched result in the requested or inferred format without hardcoded assumptions about input type, output type, document format, provider, or workflow. | PASS/PARTIAL 2026-06-15; direct MCP/API host/workspace text-input document variants produced validated professional artifacts, including a private enterprise GPT-5.4 high-effort run with DOCX/PDF primary outputs plus HTML/Markdown supporting files. Browser, PDF, DOCX, logs, DB state, model/effort, and run-script evidence were captured privately. 2026-06-16 enterprise browser smoke and post-fix high-effort rerun proved the repaired worker substrate, file preview path, View / Steer handoff, and default high-effort projection for an ordinary file task; they do not replace the full wildcard deep-work document case. Remaining surfaces/variants are authenticated LibreChat full-prompt rerun, direct UI rerun, live Telegram dedupe rerun, PPTX/file-input variants, export/recall sanitization, and renderer QA for hidden signed-url/accessibility serialization. |
...
- Last run: UPDATED 2026-06-20; local short-ref regressions added for durable default, auth gate,
...
- Last run: PASS/PARTIAL 2026-05-26 local and live enterprise automated/direct-MCP coverage. Local
  `uv run pytest -q` passed for runtime/API/MCP/bootstrap regression coverage including Codex
  runtime label, structured provider failure classification from stdout, failed-run partial
  artifact links, wait timeout config, continuation effort override, continuation de-duplication,
  provider effort fallback, fresh artifact promotion, nested `index.html` path preservation, and
  stale artifact non-promotion. A follow-up hardening pass added explicit retryable
  `runtime_io_failed` classification for closed stdin/TTY worker-session failures and verified fresh
  user-facing artifacts can still be delivered for that failure class. A live enterprise direct-MCP
  run completed a complex workspace recovery as `codex-cli`/Docker and returned artifact links for a
  Markdown report, spreadsheet-ready CSV, and final delivery manifest; authenticated browser QA
  remains required when a signed-in browser profile is available.
...
- Automation: `tests/test_mcp_server.py::test_worker_delegate_once_blocks_missing_host_cli_before_api_calls`,
  ...
  complete.
- Last run: UPDATED 2026-05-27; missing-CLI automation exists. Too-old runtime version recovery and
...
- Last run: PASS/PARTIAL 2026-06-20. Local automated runtime/UI regressions passed for short-link
  refs, compact default MCP outputs, diagnostics-only lineage, log redaction, backend/profile truth,
  and capacity retry race hardening. Local Playwright happy-path QA initially caught a raw `gh_token`
  address-bar leak after `/api/launch`; the follow-up fix switched signed UI launch/bootstrap links
  to `/r/{ref}`, set a scoped worker cookie, stripped the final redirect URL, and reran successfully
  with a tokenless `/watch/<worker>?project_id=<project>&surface=desktop` page, visible
  `codex-cli workspace - Completed`, DB `profile=codex-cli` and `backend=codex-cli`, and UI access
  logs showing `/r/{ref}` then tokenless `/watch`/`/api/worker/.../live` polling. The local degraded
  matrix also passed for timeout, auth failure, cancellation, steer/continue, quota/concurrency,
  long-wait cap, and profile fallback cases. ClaudeViv review flagged log-filter install wiring,
  runtime `/r` raw-token redirect, broad `/r` auth-prefix matching, unauthenticated health wording,
  cookie-token redaction, live manifest ambiguity, and QA overclaiming as blockers or risks. The
  applied fix added child-logger/extra-field/cookie redaction tests, runtime tokenless redirect and
  cookie coverage, narrowed the unauthenticated runtime prefix to `/r/`, removed the diagnostic
  plumbing backend from health, reconciled the 13-file live dependency manifest, and redeployed only
  the remaining GlassHive deltas after backup. Approved-enterprise live QA then passed after the
  final GlassHive-only VM hotfix: Chrome happy path showed compact `workspace_launch`/`workspace_wait`
  payloads with short refs only, workspace short link redirected to a tokenless completed
  `codex-cli` watch page, artifact short link rendered the generated markdown preview and exact
  synthetic marker, DB metadata showed Codex profile/backend/runtime, direct `X-GlassHive-*` headers
  worked, unauthenticated API returned 401, default profile fallback returned Codex truth, invalid
  profile failed closed with HTTP 403, short wait returned `still_running`, a true 305.1-second wait
  returned `still_running`/`timed_out=true`, interrupt reached `interrupted`, and continuation from
  that interrupted workspace completed with a new artifact. Final service/log scan showed runtime,
  MCP, and UI active, health reporting visible `codex-cli` with no diagnostic plumbing field,
  `/redoc` and `/runs` gated with 401, and no recent raw signed-token leaks. Live quota saturation
  was not forced because it would intentionally exhaust production capacity; local quota regressions
  cover the failure shape, and a forced live quota breach remains a maintenance-window or disposable
  tenant follow-up. ClaudeViv final review confirmed the code/live behavior but flagged release
  durability as still partial because the nested GlassHive working tree and live VM hotfix are not
  yet captured in a committed, reproducible revision; a commit/pin/manifest reconciliation is
  required before calling this fully production-grade.
