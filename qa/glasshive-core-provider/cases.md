...
| GCP-036 | Compiled and tracked provider capability metadata carries the canonical GlassHive default/full-access policy, so a prompts-only Agent sync validates existing full-access agents without changing protected workspace options | Compiler, LibreChat source/config, Agent sync dry-run/live compare | PARTIAL (2026-08-12 narrow three-Agent prompts-only update and fresh compare passed with protected runtime/model/voice/GlassHive choices preserved; post-compiler activation pending) |
