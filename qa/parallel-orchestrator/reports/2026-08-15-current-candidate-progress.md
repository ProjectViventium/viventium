...
## 2026-08-24 Continuation Addendum

- Result remains **PRE-GATE / NOT READY**. The installed local release gate accepted the exact live
  owner binding and reported **0/80 required gates PASS**. No release enablement is claimed.
- One installed Telegram Main turn produced one visible answer and matching native-provider
  authority receipt. The receipt proved one Feelings capsule, one Main instruction, and the selected
  provider/model for that mode. It does not prove every Main mode or fallback.
- Telegram readiness now requires a bot-issued, PID-bound marker. Optional command/description
  metadata runs outside the readiness path and cannot kill polling after a metadata timeout.
- LibreChat partial-stack repair now starts each launchable missing peer even when another peer's
  occupied port is unhealthy.
- Runtime-owner changes are transactional. A failed/transient start restores only a still-live
  prior owner, removes its own state when no prior owner exists, and does not overwrite a successor.
- Foreground `INT`/`TERM` now exits before one `EXIT` cleanup. Startup cannot continue creating
  watchdogs after reporting that services stopped.
- Current focused evidence: Telegram **546/546**; owner/signal/partial-stack focused release checks
  **8/8**; full production build and earlier component suites remained green. A full parent release
  rerun completed with **1,755 passed, 8 skipped, and 2 failures**. One failure correctly blocks the
  stale unrun QA backlog. The other found a stale Telegram transcription source assertion; its
  focused contract now passes **6/6** after the assertion was aligned with the guarded callback.
- Current external/runtime blockers: Telegram Bot API access unavailable during the final restart;
  authenticated Web Parallel Work unavailable; data-volume use above the release threshold; dirty
  source, nested pins, and installed artifact identity do not match.
- The exact original two-worker Telegram sequence, pre-send steer race, restart recovery, and two
  visible browser artifacts remains `NOT_RUN`. `PWK-UC-014`, `TR-026`, and `EMO-UC-047` remain open.
