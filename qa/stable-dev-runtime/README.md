...
- when Recall/RAG is explicitly local, its vector-database host port and Compose project differ
  from local prod
- both Workbench pages are visibly reachable and their watchdog PIDs remain stable during coexistence
- the supported dev stop closes only the dev runtime's ports, PID-owned sidecars, and local containers;
  pre/post prod PIDs and prod RAG container IDs remain unchanged
- stale MongoDB, Meilisearch, or LiveKit PID files are rejected unless live process identity matches
