...
from workers_projects_runtime.bootstrap import apply_bootstrap  # noqa: E402
...
INPUT_MARKER = "GLASSHIVE_INPUT_QA_MARKER"
...
def _read_input_summary(workspace: Path) -> str:
    uploads = workspace / "uploads"
    expected = [uploads / "user-brief.txt", uploads / "user-table.csv"]
    lines: list[str] = []
...
def _write_artifacts(workspace: Path, *, input_summary: str = "") -> None:
    ...
    (output_dir / "input-summary.md").write_text(
        f"# Input Materialization\n\n{PUBLIC_MARKER}\n{INPUT_MARKER}\n\n{input_summary or 'No input summary recorded.'}\n",
        encoding="utf-8",
    )
...
class SyntheticQaRuntime:
    ...
    def ensure_worker_ready(self, worker: dict[str, Any]) -> RuntimeInfo:
        ...
        apply_bootstrap(
            ...
            runtime_name=str(worker.get("profile") or "codex-cli"),
            worker=worker,
    ...
    def run_task(
        ...
        if "GH_QA_SLEEP" in instruction:
            time.sleep(30)
        input_summary = _read_input_summary(workspace)
        _write_artifacts(workspace, input_summary=input_summary)
        ...
        return (
            ...
            f"- Materialized and read public-safe input files containing {INPUT_MARKER}.\n"
            f"{input_summary}\n"
...
def build_fixture(state_root: Path) -> tuple[Any, dict[str, Any]]:
    ...
    source_root = state_root / "trusted-sources"
    source_root.mkdir(parents=True, exist_ok=True)
    source_csv = source_root / "user-table.csv"
    source_csv.write_text(f"name,status\n{INPUT_MARKER},source-path-pass\n", encoding="utf-8")
    os.environ["WPR_BOOTSTRAP_SOURCE_ROOTS"] = str(source_root)
        ...
        json={
            ...
            "bootstrap_bundle": {
                "files": [
                    {
                        "scope": "workspace",
                        "path": "uploads/user-brief.txt",
                        "content": f"{INPUT_MARKER} inline-content-pass",
                    },
                    {
                        "scope": "workspace",
                        "path": "uploads/user-table.csv",
