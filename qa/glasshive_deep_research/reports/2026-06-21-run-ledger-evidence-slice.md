...
| `GHDR-004` | PASS for local approval scope | `tests/test_profile_runtime.py`, MCP wait/status tests, active-run heartbeat from real long Codex/Claude benchmark runs, `live_provider_wait_continue_smoke.py`, `live_provider_browser_wait_continue_qa.py`, local `/w/{ref}` browser action QA | Host timeout/interrupt regressions pass; both private long runs recorded active `running` heartbeats with timeout policy and transcript paths. API/MCP wait/status/continue degraded regressions pass, and real host-Codex/Claude API wait/continue smokes observed `running -> completed` for first runs and continuations. Fresh Playwright local `/w/{ref}` browser controls proved pause, resume, interrupt, terminate, refresh persistence, invalid-ref rejection, and matching events/state with no positive-path console/request failures. The provider-backed Codex and Claude host browser bridges passed active-state/reload, artifact preview, short-ref workspace navigation, continuation, evidence, transcript-metadata, and redaction checks. Cloud/deployment browser reruns are out of scope for this local pass. |
...
| `GHDR-UC-004` | Trigger timeout/interruption and recover. | Automated host timeout/interrupt tests, real host-Codex/Claude API wait/continue smokes, and provider-backed Codex/Claude browser bridges | PASS for local approval scope | Browser observed active and completed Codex and Claude host states, refreshed while running, opened the artifact preview, followed the short-ref workspace link, and continued the same workspace. | Timeout/interrupt evidence JSON regressions passed. Live host-Codex and host-Claude API smokes observed `running -> completed` for initial and continuation runs. The provider-backed Codex and Claude browser bridges recorded `evidence_result=pass` and transcript metadata. | Cloud/deployment browser reruns are out of scope for this local pass. |
...
- Residual outside the local approval scope: live Docker Claude completion with a real headless token, cloud/deployment browser rerun, live cancellation/quota/profile fallback beyond the local synthetic/degraded slice, private long-form source-window rerun, private benchmark-specific source adjudication, and future nested-repo release/commit/pin hygiene. The artifact/tool short-ref path no longer exposes raw signed-link tokens and now opens `/w/{ref}` for member-facing workspace controls, but diagnostic/internal built-in project/view routes still use `wrk_` and `prj_` ids where directly visited.
...
| Not run / blocked | Docker Claude model task completion until a real `CLAUDE_CODE_OAUTH_TOKEN` or equivalent approved credential is present, cloud/deployment browser rerun, live quota/profile-fallback matrix beyond the local synthetic/degraded slice, private long-form post-contract source-window rerun, private benchmark-specific source adjudication, and future nested-repo release/commit hygiene. |
...
- Substitution check: automated tests, logs, DB/state, evidence JSON, live host-Codex/Claude API wait/continue, provider-backed Codex/Claude browser wait/continue, and fresh Playwright support the local browser/user path; they do not substitute for cloud/deployment reruns, live Docker Claude headless-token completion, and broader private benchmark reviews.
...
- Residual risks outside the local approval scope: source-window compliance post-fix private rerun, cloud/deployment browser rerun, live cancellation/quota/profile fallback beyond the local synthetic/degraded slice, live Docker Claude headless-token completion, private benchmark-specific adjudication, and future nested-repo release/commit hygiene.
