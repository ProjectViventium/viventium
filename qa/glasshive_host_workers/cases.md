...
- Last run: NOT YET RUN (cataloged 2026-06-10; run before approval).
...
- Last run: PARTIAL/PASS (2026-06-15 targeted policy/prompt/command regression tests passed after
  source update; live rebuilt `docs2` image policy/profile probe passed before the reproducibility
  hardening that bumped the image to `docs3`; attempted `docs3` build/run was blocked by Docker
  storage exhaustion and must be rerun after cleanup; an initial live workstation probe
  showed profile install pending while Docker overlay `/tmp` was full; after projecting browser
  temp/cache/config into the mounted worker home, the live workstation probe reported both extension
  IDs `profile-installed` within 20 seconds. Live CLI/app bridge connection QA remains required
