...
| `QASYS-024` | `GOV-010`/`GOV-015`: remove unnecessary first-use learning, inputs, choices, clicks, and words; preserve advanced capability on demand | A new user completes the default path and ordinary recovery without technical knowledge or needless setup; errors and authority remain truthful | Real browser/desktop at supported states | Before/after input, click, visible-choice and wording inventory; screenshots; responsive/theme/keyboard/a11y interaction matrix | NOT RUN — cataloged 2026-08-30 |
...
| `QASYS-UC-018` | Start without product knowledge, complete the default task, add/use files when applicable, inspect the result, recover from an ordinary failure, then discover an advanced option; repeat at supported widths, themes, and keyboard/accessibility states. | `GOV-010`, `GOV-015` / `QASYS-024` | real browser or desktop surface | before/after required inputs, clicks, visible choices, confusing wording, screenshots/DOM, focus, contrast, console, persistence, and exact result evidence | No unnecessary learning or setup step blocks first use; normal actions stay obvious and advanced power stays available without clutter. Errors and consent are preserved; reviewer opinion cannot substitute for use. | NOT RUN — cataloged 2026-08-30. |
