    ...
    await page.waitForFunction(
      () => {
        ...
        const status = [...document.querySelectorAll('[role="status"]')]
          ...
          .join(" ")
          .toLowerCase();
        const settledVisibleState = ["listening", "speaking", "working", "needs input"].some(
          (state) => status.includes(state),
        );
        const peerConnected = (globalThis.__viventiumQaPeerConnections || []).some(
          (entry) =>
            entry.peerConnection.connectionState === "connected" &&
            ["connected", "completed"].includes(entry.peerConnection.iceConnectionState),
        );
        return Boolean(
          endButton && !endButton.disabled && settledVisibleState && peerConnected,
        );
      },
      undefined,
      { timeout: 45000 },
