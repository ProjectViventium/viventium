...
| `AGCFG-005` | Every channel preserves an eager GlassHive gateway and scoped deferred discovery without semantic prompt guards. | Short and long requests can launch or check GlassHive from web, Telegram, and voice without a false unavailable claim. | source/live agent config, LibreChat web, Telegram, Modern Playground voice, MCP/logs/DB/state | release/Jest regressions plus exact-model real-surface QA | PASS 2026-07-13; see `reports/2026-07-13-cross-surface-glasshive-parity.md` |
...
| `AGCFG-UC-005` | Send one natural long GlassHive request followed by a terse status request through Telegram, then repeat the launch/status contract in LibreChat web and a real voice call. | `03_Telegram_Bridge.md`, `07_MCPs.md`, `37_LibreChat_v083_Config_Alignment.md` / `AGCFG-005`, `TR-008`, `MPV-014` | Telegram, authenticated Chrome chat, Modern Playground call | provider-bound tool names, `tool_search`/tool calls, visible and audible results, Mongo content parts, GlassHive run/events, runtime logs, restart/reload evidence, latency | All three channels expose the eager launch/status/wait gateway; deferred tools remain discoverable in the same invocation; no channel claims GlassHive is unavailable while the server is healthy. | PASS 2026-07-13; native Telegram, Playwright LibreChat, and real voice→GlassHive evidence captured |
...
- Last run: PASS 2026-07-13; see
  `reports/2026-07-13-cross-surface-glasshive-parity.md`.
