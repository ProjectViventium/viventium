...
| Feelings and the Emotional Reaction Cortex | [`54_Emotional_Cortex_And_Feeling_State.md`](../docs/requirements_and_learnings/54_Emotional_Cortex_And_Feeling_State.md) | `qa/emotional-cortex/` | User opens the first-run off state, enables Feelings, adjusts Current/Nature/return speed, omits and restores a band, edits reaction activation/instructions, sends a synthetic stimulus, sees the reply remain unblocked, observes later typed state change/health, refreshes/restarts, tests stale edits and degraded model output, and verifies the same pinned state across every configured surface/agent scope. |
...
| Parallel Work orchestration | [`55_Parallel_Work_Orchestration.md`](../docs/requirements_and_learnings/55_Parallel_Work_Orchestration.md) | `qa/parallel-orchestrator/` | User toggles the account-wide setting; sends rapid A/B/C; inspects and controls exact work from Web, Telegram, and Voice; handles missing auth/capacity/provider failures; receives callbacks after moving on or restart; runs scheduled required work; tests native child topology, two-owner isolation, maximum load, clean install, and rollback without losing retained work. |
...
`qa/parallel-orchestrator/`, `qa/prompt-workbench/`, `qa/qa-system-audit/`, `qa/red-team-cortex/`, `qa/release-readiness/`,
