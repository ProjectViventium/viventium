...
Status: retired / historical comparison only. Current connected-account acceptance belongs in
`qa/glasshive-mcp-capability-broker/` and must prove the GlassHive broker-first path.
...
| --- | --- | --- | --- | --- | --- |
| CA-HANDOFF-001 | Historical hand-off experiment | "any new emails today?" hands off to Connected Accounts, reads **both** Gmail and Outlook, returns a concise summary inline in one turn — no GlassHive worker, no clarification, no plumbing link. | LibreChat Web UI | Manual/browser | HISTORICAL PASS 2026-05-28 (`lc_transfer_to_...` -> `search_gmail_messages` + `list-mail-messages`, ~35-45s, no leak). Not current acceptance. |
...
- Recreating the Connected Accounts hand-off edge as the current product path instead of using the
  GlassHive broker-first worker path.
- Falling back to browser/computer use when the brokered MCP path is available and suitable.
- Surfacing a forced download, `[local worker link]`, or other worker/run plumbing in the reply for a quick read.
...
| --- | --- | --- | --- | --- | --- | --- |
| `CA-HANDOFF-UC-001` | Ask for a same-day inbox rundown and compare the retired Connected Accounts handoff with the broker-first GlassHive path. | `48_GlassHive_Workstation_Sandbox_Runtime.md` / `CA-HANDOFF-001`, `GH-MCP-BROKER-014` | LibreChat browser chat plus current GlassHive broker QA report | Historical handoff result, current broker run status, provider/auth blockers, and public-safe transcript summary | The current acceptance path is broker-first; any historical handoff note is labeled comparison-only and does not re-enable the retired route. | HISTORICAL 2026-05-29; current acceptance moved to `qa/glasshive-mcp-capability-broker/` |
| `CA-HANDOFF-UC-002` | Try a connected-account write request and verify the retired handoff cannot silently mutate email or calendar content. | `40_Public_Private_Boundaries_and_License_Matrix.md` / `CA-HANDOFF-004` | LibreChat browser chat, agent/tool config inspection, broker confirmation path | Read/write tool list, confirmation requirement, visible user wording, and sanitized logs | Reads remain separated from writes; writes require the current broker/confirmation path or an honest blocker. | PARTIAL 2026-05-28; config proof passed, live write-routing exercise pending |
