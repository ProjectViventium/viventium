...
- Last run: PARTIAL 2026-07-14; 80 proposal/hardener tests plus live guarded CAS
...
- Last run: PASS 2026-08-09, with current scheduled parity proved 2026-08-21 under
  `MEMHARD-015`. Luna/medium and Sol/xHigh each passed 32/32; Terra/high
  ...
  workpack. Real browser and Telegram writer/read paths also selected Luna/medium. The naturally
  due schema-v3 LaunchAgent receipt now proves the joined configured/requested/effective tuple.
...
- Last run: PASS 2026-08-21. The failure-first stale-receipt test passed, then the naturally
  due LaunchAgent run carried schema-v3 PID/PPID attestation and completed on the configured
  OpenAI/Luna/medium tuple. Requested, effective, and configured execution now agree and the prior
  mismatch is cleared without changing the route.
...
| --- | --- | --- | --- | --- | --- | --- |
| `MEMHARD-UC-001` | On memory hardener, reports, runtime env, synthetic memories, verify that memory hardening runs are bounded, public-safe, and preserve durable facts while pruning stale/private noise. | owning requirement for `MEMHARD-001` / `MEMHARD-001` | memory hardener, reports, runtime env, synthetic memories | Source, owning requirement doc, case steps, logs, DB/state, generated config, and shipped artifact evidence that apply to MEMHARD-001. | User-visible behavior matches source, docs, persisted state, and logs | PASS 2026-06-11 ([nightly review](reports/2026-06-11-nightly-routines-health-review.md)); launchd fired and skipped on battery, while the latest model-backed run remains healthy |
...
- `tests/release/test_memory_model_eval_harness.py`
