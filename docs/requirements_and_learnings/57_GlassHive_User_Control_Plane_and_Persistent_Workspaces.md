# 57. GlassHive User Control Plane and Persistent Workspaces

The owning acceptance spine is [`qa/glasshive-user-control-plane/`](../../qa/glasshive-user-control-plane/).
The older [`qa/glasshive_workspaces/`](../../qa/glasshive_workspaces/) owner covers only the local
worker-alias workspace foundation; it does not prove hosted login, personal connections, rename, or
two-user isolation.
...
- Codex uses an account-specific `CODEX_HOME`; Claude uses its supported native authentication home
  or provider-supported headless token flow. In Docker, Claude keeps user-scope tools, plugins,
  trust, and onboarding state in the workspace's persistent native `HOME`, while only the selected
  account's native secure-storage directory is projected for the leased mission. GlassHive does not
  invent, extract, merge, or copy Claude OAuth or connector configuration.
...
- Claude's Docker mission contract composes rather than replaces native state: the workspace retains
  its own `$HOME/.claude.json` user-scope MCP/plugin/trust state, and the selected account contributes
...
- The workspace-native `Set up tools` action for Codex or Claude borrows the exact selected personal
  ...
  and shell actions never acquire provider credentials.
...
- A persisted Codex thread id or Claude session id is continuity metadata, not workspace identity. If
  the installed harness reports that the exact prior conversation no longer exists after an upgrade,
  rollback, or native-state boundary correction, the same user action starts one fresh conversation
  with the same instruction and workspace, then persists its replacement id. Other resume failures
  are not retried or hidden.
...
| Worker toolchain | Existing host-native compatibility floors are Codex `0.144.1` and Claude Code `2.1.178`; the generated workstation source separately pins Codex `0.147.0`, Claude Code `2.1.233`, the Selenium base image by exact digest, and OpenClaw `2026.7.1-2` through its reviewed package lock, with capability preflight on both paths | Clean-bootstrap/install capability proof on approved platforms; retain and re-verify the immutable Selenium and OpenClaw inputs before satisfying the provenance requirement above |
| Delivery/public safety | Isolated source checkout, public-safe synthetic test data, read-only protected downstream boundary, source/install separation, per-component SQLite schema ledger with newer-database refusal, a portable WAL backup/clone rehearsal/invariant/failure-injection/database-restore harness passed on macOS and clean Debian, runtime startup refusal for a readable signer key, verifier-only integrated runtime launch, hard runtime/MCP/BFF readiness, accepted atomic three-service canary cutover, exact installed source provenance, current nested parent pin, and current repository public-safety checks | Separate signer/runtime OS identities, complete identity-header/path-negative matrix, installed migration/cutover/browser/MCP restore and preceding-artifact recovery, clean clone/install, final staged/generated/build-artifact scan, and protected downstream owner-run compatibility |
