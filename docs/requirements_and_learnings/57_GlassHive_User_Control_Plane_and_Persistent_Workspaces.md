...
| Worker toolchain | Existing host-native compatibility floors are Codex `0.144.1` and Claude Code `2.1.178`; the generated workstation source separately pins Codex `0.147.0` and Claude Code `2.1.229`, with capability preflight on both paths | Clean-bootstrap/install capability proof on approved platforms; replace the current floating Selenium and OpenClaw image inputs with reviewed immutable pins before satisfying the provenance requirement above |
