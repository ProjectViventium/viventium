...
### Data Layer
- **Mongoose schema** (`packages/data-schemas/src/schema/agent.ts`): `voice_llm_model: { type: String, default: null }`, `voice_llm_provider: { type: String, default: null }`
- **TypeScript types** (`packages/data-provider/src/types/assistants.ts`, `packages/data-schemas/src/types/agent.ts`): `voice_llm_model?: string | null`, `voice_llm_provider?: string | null`
- **Zod validation** (`packages/api/src/agents/validation.ts`): `z.string().nullable().optional()`
- **Default form values** (`packages/data-provider/src/schemas.ts`): both default to `null`
